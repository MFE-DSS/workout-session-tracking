...
`0d01e1c`** aux 3 points. Voir §Anomalies pour les collisions de sessions détectées et résolues.
...
- **`data/method_rules.json:13`** — texte de règle de méthode **stocké en données** et injecté sur
  ...
  SPIGNOS reste donc rendue sur /science — **tolérée et documentée** (test sentinelle plafonne à ≤ 1).
...
- **Migration `data/method_rules.json`** — pass données futur (hors 10.4).
