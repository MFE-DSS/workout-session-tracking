...
| **Prochaine action** | `Sb_DOGFOOD_01.2` **accepté** ✅. Ensuite `Sb_DOGFOOD_01.3` (mobile placeholder CSS : format `102.5`, typo réduite) 🟡 READY TO BE PROPOSED — dernier volet du cycle DOGFOOD_01. Autres pistes : `Sb_UI_06.4`, reprise **Body Intelligence**, `Sb_32.4`. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
