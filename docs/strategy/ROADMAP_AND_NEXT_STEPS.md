...
| **Sx_32 Deep Feature Refactor (Muscle/BodyZone) — CYCLE MÉTIER AUTORISÉ** | ✅ **SPEC HUMAN REVIEW ACCEPTED + cycle autorisé** 2026-07-07 (commit spec `47f3fac`, `docs/SPRINT_Sx_32_HUMAN_REVIEW_REPORT.md`). Premier chantier de **refonte profonde des features/objets backend** : **modèle Muscle/BodyZone formel** en base (remplace le substring-matching de `muscle_mapping.py`), débloque Worked Area UI + coach + body intelligence. OQ-32-A→G confirmées (clé `exercise_code` · Zone V1 · muscle_mapping primaire · substitution indépendante · fallback substring conservé · stabilizers « à qualifier » · Worked Area = sprint UI séparé). **Invariance historique = contrainte #1** ; non-régression obligatoire ; migrations additive-only. Autres axes Tier 1 (readiness agg, identité exercice, substitution first-class) en backlog. |
...
| Sb_32.2 ExerciseMuscleMapping + classify lookup/fallback | 🟢 **DELIVERED — pending CI + human review** 2026-07-08 (`docs/SPRINT_Sb_32_2_REPORT.md`). Modèle `ExerciseMuscleMapping` + migration `k2l7f3g4i65` additive-only : 1 table neuve + backfill **87 lignes statiques générées au build** depuis la baseline `.1` (65 primary + 22 secondary ; `unknown` non inséré). **Découverte** : catalogue sans code d'exercice stable (`code`=slot jour E1…E7) → `exercise_code = name` → lookup DB et fallback keyés pareil → **équivalence prouvable**. `classify_exercise(name, *, exercise_code=None, db=None)` : lookup DB **optionnel** + fallback substring, **name-only byte-identique**. **Invariance prouvée : DB lookup ET name-only == baseline 91/91.** **Aucun consommateur muté** (7 callers intacts), aucune UI. Snapshot +9 l. additives ; drift/snapshot/patterns/roundtrip/spec protocol verts ; ruff 541 ≤ 548 ; 14 nouveaux tests + broad sweep 381 verts. Étape 0 Brainstorming documentée. En attente **GO commit** puis CI réelle. |
| Sb_32.3 body_map_descriptor | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-08. Contrat de représentation corporelle (agrégation zone→descripteur) consommable UI Worked Area + coach, s'appuyant sur `ExerciseMuscleMapping` (`.2`). Review-gated, sous garde de non-régression. Sur override explicite opérateur. |
| Sb_32.4 | ⏸️ **BLOCKED** (séquentiel, review-gated). Migration consommateurs (coach/body_intel/scoring) vers lookup DB. Prouve la non-régression (classify old=new) avant bascule. |
| **Prochaine action** | `Sb_32.2` **livré, en attente GO commit** (pas encore commité/pushé). Après GO commit → CI réelle 3/3, puis human review. `Sb_32.3` (`body_map_descriptor`) reste bloqué tant que `.2` n'est pas accepté. Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
