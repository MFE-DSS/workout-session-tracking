...
| Sx_UI_11 Screenshot Regression Baseline | 🟢 **SPEC delivered 2026-07-02 — pending human review** (`docs/strategy/Sx_UI_11_SCREENSHOT_REGRESSION_BASELINE_SPEC.md` + `docs/SPRINT_Sx_UI_11_REPORT.md`). Playwright Python binding recommandé. Matrice 18 écrans × 2 viewports (P0=14 obligatoires, P1=16 recommandés, P2=6 différables). Fixture DB locale déterministe, jamais dépendante prod. Auth avec règle anti-secret hard. Revue humaine primaire, aucun gate CI binaire V1. Baseline non-versionnée par défaut, storage final en artefacts CI + release tag. Aucun outil installé, aucun screenshot capturé, aucun script écrit. |
| **Prochaine action** | **GO human review Sx_UI_11**, puis choix : (Option A recommandée) ouvrir `Sx_UI_04_SESSION_FOCUS_RESKIN_SPEC` SPEC ONLY en parallèle du futur build baseline, OU (Option B) ouvrir `Sb_UI_11.1_SCREENSHOT_TOOLING_BUILD` **BUILD** immédiat (touche `scripts/`, `tests/`, potentiellement `.github/`, déclenche CI complète). **Build UI reste BLOQUÉ.** Sx_UI_04 reskin ne peut démarrer son sprint de build tant que baseline P0 (14 screenshots) n'est pas disponible OU sans dérogation opérateur explicite. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
