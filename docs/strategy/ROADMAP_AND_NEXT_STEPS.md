...
| Sb_OPS.sonar-hygiene-p1 (dédup littéraux Sonar) | 🟢 **PR OPENED / MERGE PENDING** 2026-08-07 (base `038d194`, branche `sb/ops-sonar-hygiene-p1` ; `docs/SPRINT_Sb_OPS_SONAR_HYGIENE_P1_REPORT.md` ; sous protocole agentique). 1ᵉʳ lot d'hygiène Sonar **behavior-preserving** : `python:S1192` (littéraux dupliqués, priorité #1) cluster **mono-fichier `app/routers/squads.py`** → 3 constantes (`_SQUAD_NOT_FOUND` ×8, `_ACCESS_DENIED` ×8, `_CHALLENGE_CREATE_TEMPLATE` ×3), **19 duplications éliminées**, zéro comportement/schéma/test/exclusion. `auth_routes.py` écarté (prudence « sensitive auth »). **Reste documenté** (15 S1192 · ≈450 external_ruff · 52 S9073 · 27 S3776) — réduction incrémentale, main project_status non hard-gate. Broad sweep squads **35 passés**, check_scope **ISOLATED**, budget 543 ≤ 548. **Prochaine action** : merge PR (GO humain) puis closeout ; lots hygiène suivants au besoin. |
