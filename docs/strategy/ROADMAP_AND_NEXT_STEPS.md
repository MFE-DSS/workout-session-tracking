...
| Sb_UI_10.4 User-Facing Docs / Labels Auren Pass | 🟢 **CODE COMPLETE — CI PENDING — HUMAN REVIEW PENDING** 2026-07-15 (`docs/SPRINT_Sb_UI_10_4_USER_FACING_LABELS_AUREN_PASS_REPORT.md`). String-only pass : 8 chaînes produit visibles migrées SPIGNOS → Auren (science ×5, atlas lede, coach-report note, SVG diagramme `<title>` a11y). Sens scientifique/coach/atlas inchangé ; SVG structure+`id`s intacts ; routes/backend/manifest/assets/CSS/JS/`data/` non touchés ; zéro Orion. **1 occurrence OPEN** : `data/method_rules.json` (hors périmètre `data/**`, sentinelle == 1 ; micro-pass **`Sb_UI_10.4b`** sur GO). 12 dédiés + science_page ré-orienté + surfaces = 49 verts ; ruff 543 ≤ 548 ; spec PASS. CI complète au push. |
