...
| **Prochaine action** | **Décider quel build ouvrir sur GO séparé** : (a) `Sb_BI_01.1` **Zone Intelligence Cards** 🟡 READY TO BE PROPOSED, not opened — angle Option A validé ; (b) **Transformation corpus improvement sprint** 🟡 READY TO BE PROPOSED — amélioration du corpus / mapping exercice→zone (couverture, qualité classification, socle du volume par zone). **Dogfooding terrain Sx_DOGFOOD_01** toujours pending (checklist §7 du closeout). Body Intelligence build **deferred until explicit GO**. Pistes hors cycle (aucune ouverte sans GO) : `Sb_UI_06.4`, `Sb_32.4`, release tag, smokes UI auth prod. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31, activation `/body` Manual Profile, décision score `/physique`. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
