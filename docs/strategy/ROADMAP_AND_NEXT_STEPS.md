...
| **Sx_UI_02b Auren Terminal (RÉVISION DIRECTION)** | 🟢 **SPEC delivered — pending human review** 2026-07-07 (`docs/strategy/Sx_UI_02b_AUREN_TERMINAL_SPEC.md` + `docs/SPRINT_Sx_UI_02b_REPORT.md`). Brainstorm opérateur : identité → **« Palantir du bodybuilding »** = graphite dense + tout-mono terminal + accent unique rare (ambre `#C8A24B` candidat). **Révise Sx_UI_02** (teal retiré, dark = identité). **Re-skin, pas re-architecture** — invariants Sx_UI_04 intacts. Migration **Home + Focus Mode** en 3 builds : Sb_UI_02b.1 (tokens + Home, remplace le skin teal), .2 (Focus Mode), .3 (hardening + nav). 8 OQ. Le teal `#0F8A85` (Sb_UI_04.1→.5 + hero Sb_UI_05.1) disparaît au re-skin. |
| **Prochaine action** | **Human review** de `Sx_UI_02b_AUREN_TERMINAL_SPEC` + confirmer OQ-02b-A → OQ-02b-H (surtout l'accent ambre). **Puis ouvrir `Sb_UI_02b.1`** (tokens implémentés + re-skin Home graphite/mono/ambre) sur override. `Sb_UI_05.2` et `Sx_UI_06` : différés derrière la révision visuelle. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra. |
| Ruff budget | **534 ≤ 548** |
