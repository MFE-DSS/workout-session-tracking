...
| **Migration Auren Terminal V1 (Sx_UI_02b)** | 🟢 **Home + Focus + Shell livrés & cohérents — READY FOR CLOSEOUT after review** (Sb_UI_02b.1 Home · .2 Focus · .3 Shell). Graphite/mono/ambre transverse. |
...
| **Sx_32 Deep Feature Refactor (Muscle/BodyZone) — NOUVEAU CYCLE MÉTIER** | 🟢 **SPEC delivered — pending human review** 2026-07-07 (`docs/strategy/Sx_32_MUSCLE_BODYZONE_MODEL_SPEC.md` + `docs/SPRINT_Sx_32_REPORT.md`). Ouvre la **refonte profonde des features/objets** demandée par l'opérateur (au-delà de l'UI). Premier axe : **modèle Muscle/BodyZone formel** en base (remplace le substring-matching heuristique de `muscle_mapping.py`), débloque le Worked Area UI + coach + body intelligence. Ancré sur un **audit backend read-only**. Invariance historique = contrainte #1. Découpage Sb_32.1→.4, migrations additive-only, 7 OQ. Autres axes Tier 1 (readiness agg, identité exercice, substitution first-class) en **backlog documenté**. **Override cycle métier + validation OQ requises avant tout build.** Docs-only, aucun code. |
| **Prochaine action** | **Human review** de `Sx_32_MUSCLE_BODYZONE_MODEL_SPEC` + trancher OQ-32-A→G + **override d'ouverture du cycle métier**. Puis **ouvrir `Sb_32.1`** (BodyZone+Muscle+backfill). — Sb_UI_02b.3 (shell terminal) livré + CI verte (run `28882063535`), migration Auren Terminal V1 ready for closeout. Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`. |
| Ruff budget | **534 ≤ 548** |
