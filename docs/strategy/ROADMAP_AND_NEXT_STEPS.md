...
- **Nouveau track — Morphology Program** : `Sx_MORPHO_PROGRAM_01` (architecture, **SPEC-ONLY**) = ✅
  **MERGED + VERIFIED** (`8d1ce98`). File de build (chacun sur GO explicite, dans l'ordre des dépendances) :
  ...
  3. **`Sb_MORPHO_PROGRAM_GENERATOR_01`** — générateur déterministe morpho additif + availability Fitness Park = ⏳ **PR GREEN / MERGE PENDING** (base `1b1d36d`, branche `sb/morpho-program-generator-01` ; `app/services/morpho_program_generator.py` pur/déterministe : descripteurs/priorités → `SlotIntent`s ordonnés + exercices EKB candidats via `compute_proximity` **lecture seule** + pool `exercise_properties` + availability, fingerprint sha256 `mpg1-` ; **zone macro `lower` désambiguïsée par `muscle_group`** → mollets absents du pool = **warning de couverture, 0 fabrication** ; 18 tests + broad sweep ciblé 180, **substitution N1/N2/N3 inchangée** ; 0 DB/migration/publication/hardcoding Martin ; merge = **GO humain**) ;
