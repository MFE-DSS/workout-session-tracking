...
| Sx_UI_02 Design Tokens | 🟢 **SPEC delivered 2026-07-02 — pending human review** (`docs/strategy/Sx_UI_02_DESIGN_TOKENS_SPEC.md` + `docs/SPRINT_Sx_UI_02_REPORT.md`). Accent principal = **teal chirurgical désaturé** (OQ-B tranché), fallback = bleu minéral. Palette + typo + spacing + radius + shadows + motion + chart tokens documentés en candidats. Valeurs hex non figées (OQ-H). Prochaine action = **human review Sx_UI_02**. |
| Sb_OPS.ci-path-filter | ✅ **DELIVERED 2026-07-02** (commit `a9ab10c`, CI run `28582551168` ✅ success). `paths-ignore: ['docs/**']` sur trigger `push`. Économie CI attendue ~90% sur les 9 specs Sx_UI docs-only restantes. `pull_request` et `deploy-production.yml` non affectés. |
| Ruff budget | **534 ≤ 548** |
