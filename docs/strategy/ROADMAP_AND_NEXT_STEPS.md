...
| **Prochaine action** | **GO `Sb_UI_11.1_SCREENSHOT_TOOLING_BUILD` BUILD.** Sur override explicite opérateur. Sprint OPS hors-cycle. Touche `scripts/`, `tests/`, potentiellement `.github/workflows/` et `requirements-lock.txt`. **Déclenchera CI complète au push** (comportement voulu, ~22-25 min compute). Livrable : install Playwright + fixtures DB + capture P0 (14 screenshots). **Sb_UI_04.1 reste BLOQUÉ** tant que Sb_UI_11.1 n'a pas livré la baseline P0. **Aucune dérogation baseline n'est accordée.** Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
