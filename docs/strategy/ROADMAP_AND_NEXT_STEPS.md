...
| Sx_UI_07.1 Progress Surface Auren Readability | 🟢 **DELIVERED — pending GO commit + CI + human review** 2026-07-11 (`docs/SPRINT_Sx_UI_07_1_PROGRESS_AUREN_READABILITY_REPORT.md`). **Option A pure** (template-only sur `/progress`) : lede utile, section « Rythme récent », titres timelines sobres, note « Lecture indicative ». Tous les blocs conservés ; **aucun service touché** (`kpis.py`/`timeline.py`/`weekly_loop.py`/`pages.py` non ouverts) ; aucune valeur/calcul changé ; aucun JS/score/lien BI-physique/couleur/rebrand. 11 tests dédiés + broad sweep 232 verts. Labels KPI « sessions » **conservés** (2 assertions textuelles de `test_kpis.py` hors périmètre y sont asservies → décision : rester dans le périmètre `progress.html` seul). check_scope=ISOLATED. **CI fix `ci_infra`** (`SPRINT_Sx_UI_07_1_CI_TIMEOUT_HEADROOM_REPORT.md`) : run `29239935779` cancelled par timeout job (35:17 > 35 min, step pytest cancelled à 34:46) malgré **1978 passed** → patch `timeout-minutes: 35 → 45` (build `98d679d` valide, template-only ; aucun gate/coverage/QA/Sonar retiré). |
| **Prochaine action** | **GO commit Sx_UI_07.1** attendu (build de code → CI réelle après push), puis human review. Ensuite sur GO séparé : `Sx_UI_07.2` **History Surface Auren Readability** (même approche template-only sur `/history`), ou alignement FR complet des labels KPI (touche `test_kpis.py`, futur sprint). `Sx_UI_08.3` Service Worker / Offline Session Spec ⏸️ deferred. **Séquence recommandée** : (1) **Dogfooding terrain Sx_DOGFOOD_01** (pending, en différé) — valider les surfaces BI + cohérence charge **avant** de les rendre publiques ; (2) si dogfood concluant → **GO activation** : futur build/ops `Sb_BI_01.activation` (poser `BODY_INTELLIGENCE_ENABLED=1` en prod + ajouter `/body/intelligence` au smoke + deploy `ref=<SHA réel>`) — **GO deploy explicite requis**, smokes jamais sur compte prod. Le plan readiness est prêt (`Sb_BI_01_ACTIVATION_READINESS_PLAN.md`). Autres pistes GO : volume/exercice drill (future spec), corpus improvement (non bloquant). Body Intelligence activation / deploy **deferred until explicit GO**. Pistes hors cycle : `Sb_UI_06.4`, `Sb_32.4`, `Sx_UI_07` (Progress), `Sx_UI_08` (PWA), release tag, smokes UI auth prod. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31, activation `/body` Manual Profile, rebrand `Sx_UI_10`. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
