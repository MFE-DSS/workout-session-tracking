...
| Sb_32.next.worked-area-descriptor-ui | 🟢 **DELIVERED — pending CI + human review** 2026-07-09 (`docs/SPRINT_Sb_32_next_WORKED_AREA_DESCRIPTOR_UI_REPORT.md`). **Premier consommateur visible** de `body_map_descriptor` : route `session_detail` injecte le descriptor par exercice ; Worked Area (carte active) rend **primary/secondary labels réels** (Chest Press → Pectoraux/Triceps), unknown → `"À qualifier"`, `resolution_path` en `data-*`, note prudente non médicale. **SSR/no-JS.** Aucun service métier/modèle/migration/schema/endpoint/JS/asset touché ; contrats Focus Mode préservés. Garde-fou : tier `isolated` (full sweep local skippé, broad sweep élargi joué). ruff 541 ≤ 548 ; spec vert ; 14 tests + broad sweep verts. Étape 0 Brainstorming documentée. En attente **GO commit** puis CI réelle. |
| Sb_32.4 | ⏸️ **BLOCKED / DEFERRED** 2026-07-09. Migration consommateurs (coach/body_intel/scoring) vers lookup DB + descriptor. Review-gated, prouve la non-régression (classify old=new) avant bascule. |
| **Prochaine action** | `Sb_32.next.worked-area-descriptor-ui` **livré, en attente GO commit**. Après GO commit → CI réelle 3/3, puis human review. Ensuite arbitrage : `Sb_32.4` (bascule consommateurs métier) **ou** closeout Sx_32 (partial foundation : fondation + 1er consommateur UI livrés, bascule métier différée). Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
