...
| **Prochaine action** | ✅ **Sx_DOGFOOD_01 audité + spec** (READY FOR HUMAN DECISION). Sur GO : ouvrir `Sb_DOGFOOD_01.1` (`last_time` substitution-aware, Option A, minimal). Autres pistes : `Sb_UI_06.4` (écrans secondaires), reprise **Body Intelligence**, `Sb_32.4` (bascule consommateurs métier). Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
