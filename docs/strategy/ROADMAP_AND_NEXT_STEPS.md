...
| Sb_UI_04.5 Worked Area Visual Slot + Alternatives + Hardening | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-06. Enrichissement visuel muscle/zone (contrat `body_map_descriptor` §23.5, placeholder clinique V1, pas de GIF/asset obligatoire) + surface Alternatives (substitution rendue produit, affichage seul) + hardening visuel/a11y + closure Sx_UI_04. Sans modèle ni pipeline média. |
| **Prochaine action** | Sb_UI_04.4 **accepté pour continuation** ✅. **Ouvrir `Sb_UI_04.5 Worked Area Visual Slot + Alternatives Surface + Hardening` SPEC-then-BUILD** sur override explicite opérateur (enrichissement Worked Area, surface Alternatives, hardening, closure Sx_UI_04 — sans modèle ni pipeline média). Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warning bcrypt. |
| Ruff budget | **534 ≤ 548** |
