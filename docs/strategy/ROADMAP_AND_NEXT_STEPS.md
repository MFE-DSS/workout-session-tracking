...
| **Sb_ASSET_02.1 Vendored Icon Subset & License Intake** | 🟢 **CODE COMPLETE — CI PENDING — HUMAN REVIEW PENDING** 2026-07-20 (baseline `fe97adc` ; `docs/SPRINT_Sb_ASSET_02_1_VENDOR_ICON_SUBSET_LICENSE_INTAKE_REPORT.md`). **PREMIER INTAKE TIERS** : 10 SVG **Tabler v3.45.0** (commit `975920ff…`, pins vérifiés) + **licence MIT byte-identique** (`tabler-MIT.txt`). Normalisation bornée (commentaire+LF+newline, géométrie inchangée, prouvée indépendamment). Semantic map + registre machine-lisible (preuves blob SHA/sha256 par fichier) + manifest 10 entrées (0 approved) + provenance (`vendor`, `NOT AUREN IP OWNERSHIP`, 0 verified) + preview statique. Garde évolué (allowlist SVG exacte, LICENSES = README+tabler-MIT). 20 + 25 tests stdlib. **0** Health Icons/custom/P1/`app/**`/dépendance. **`ASSET INTEGRATION GATE: BLOCKED`** — 10 icônes NOT AUTHORIZED FOR APP INTEGRATION. **Prochaine action : `GO VALIDATE — Sb_ASSET_02.1`** (non commencé). |
