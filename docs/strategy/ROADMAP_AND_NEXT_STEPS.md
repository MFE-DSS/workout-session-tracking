...
| Sb_UI_02b.3 Hardening + Shell/Nav | ⏸️ **next candidate, not opened** (post-Sb_UI_02b.2 CI + review). Polish AA cross-écran + migration shell/bottom nav (Sx_UI_03) → Auren Terminal + baseline P0 re-capturée. Closure migration Sx_UI_02b. |
...
| **Prochaine action** | **Vérifier CI verte** sur push Sb_UI_02b.2 (3 jobs, Sonar Java 21 déjà fixé). **Puis human review** (comparaison `var/visual-after/Sb_UI_04_5/session-detail-active-*` teal vs `Sb_UI_02b_2/session-detail-active-*` terminal — critère : « Home et Focus même monde graphite/mono/ambre »). **Puis `Sb_UI_02b.3 Hardening + Shell/Nav`** sur override (closure migration). `Sb_UI_05.2` et `Sx_UI_06` différés. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra. |
| Ruff budget | **534 ≤ 548** |
