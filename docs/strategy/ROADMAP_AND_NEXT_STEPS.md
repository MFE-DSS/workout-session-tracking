...
| Sx_UI_03 App Shell Navigation | 🟢 **SPEC delivered 2026-07-02 — pending human review** (`docs/strategy/Sx_UI_03_APP_SHELL_NAVIGATION_SPEC.md` + `docs/SPRINT_Sx_UI_03_REPORT.md`). Bottom nav V1 = 4 entrées (Séance / Programmes / Progression / Profil). Coach contextualisé, Squads/Classement dans Profil, Historique/Physique sous Progression. Desktop = rail latéral gauche. OQ-C/D/E/N/O/P/Q/S/T/U tranchées V1. OQ-R (sous-nav Progression) reste pending pour Sx_UI_04. |
| **Prochaine action** | **GO human review Sx_UI_03**, puis choix : (Option 1 recommandée) ouvrir `Sx_UI_11_SCREENSHOT_REGRESSION_SPEC` SPEC ONLY pour produire baseline avant Sx_UI_04, OU (Option 2) ouvrir `Sx_UI_04_SESSION_FOCUS_RESKIN_SPEC` SPEC ONLY en parallèle de Sx_UI_11. **Build UI reste BLOQUÉ.** `Sx_UI_04` (premier sprint autorisé à toucher du code visuel) exige `Sx_UI_11` baseline disponible + OQ résiduelles Sx_UI_02 tranchées. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
