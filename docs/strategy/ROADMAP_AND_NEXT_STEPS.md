...
| Sb_UI_04.3 Active Exercise Cockpit Shell | 🟢 **DELIVERED — pending CI + human review** 2026-07-05 (`docs/SPRINT_Sb_UI_04_3_REPORT.md`). Cockpit wrapper + orientation (X/Y + N restants) + mini-stepper compressé (remplace jump bar dense, OQ-B) + carte active hero dominante (cartes non-actives en index secondaire) + Worked Area Panel (zone principale = atlas family réelle, assistants/stabilisation fallback conservateur, note anti-médicale) + exercise intent + technical cues (max 3, atlas execution_cues) + up-next enrichi zone (OQ-F). SSR/CSS pur, **aucun JS ajouté**, anchors `#exercise-N` + no-JS + forms logging + rest timer `data-*` préservés. 285 tests focus/visual verts + **34 nouveaux tests cockpit** ; ruff 542 ≤ 548 ; after-capture locale **16/16 ok=16 failed=0** (delta +27% session-detail-active vs Sb_UI_04.2). Whitelist respectée (2 templates + CSS scoped + 2 tests). |
| Sb_UI_04.4 Set Logging Console + Progression Guidance | ⏸️ **next candidate, not opened** (post-Sb_UI_04.3 CI + review). Saisie sets instrumentale + previous perf + target range + hint overload (présentation). |
...
| **Prochaine action** | **Vérifier CI verte** sur push Sb_UI_04.3. **Puis human review** (comparaison visuelle after-capture opérateur `var/visual-after/Sb_UI_04_3/` vs `Sb_UI_04_2/` — critère : « cockpit, plus une liste repeinte »). **Puis proposition `Sb_UI_04.4 Set Logging Console + Progression Guidance`** sur override explicite. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warning bcrypt. |
| Ruff budget | **534 ≤ 548** |
