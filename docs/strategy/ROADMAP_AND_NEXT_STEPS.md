...
| Sb_32.1 BodyZone + Muscle Foundation | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-07. Modèles `BodyZone`+`Muscle` (additive-only) + backfill des 11 zones/labels/measurement/radar/volume → base + test d'invariance. Aucun consommateur muté (bascule `classify_exercise` en `.2`). DoR : baseline non-régression capturée avant. |
| Sb_32.2 → .4 | ⏸️ **BLOCKED** (séquentiels, review-gated). ExerciseMuscleMapping · body_map_descriptor · migration consommateurs. |
| **Prochaine action** | Sx_32 **accepté + cycle autorisé** ✅. **Ouvrir `Sb_32.1 BodyZone + Muscle Foundation` SPEC-then-BUILD** sur override explicite opérateur (premier vrai code métier de la refonte). Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
