...
| Sb_UI_11.2 Baseline Runtime Integration Patch | 🟢 **DELIVERED 2026-07-02 — pending human review + CI complète** (`docs/SPRINT_Sb_UI_11_2_BASELINE_RUNTIME_PATCH_REPORT.md`). CLI `visual_baseline_runtime.py prepare/verify` intégré à l'app existant (get_settings, SessionLocal, instantiate_session, cookie signé). Refus prod + refus DB non-locale. Patch `capture --runtime-file`. 145 tests. **Élimine `.env.baseline` manuel** — capture opérateur en 2 commandes simples. |
