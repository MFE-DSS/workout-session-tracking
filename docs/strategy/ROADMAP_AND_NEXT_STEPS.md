...
| Sx_UI_05 Today / Readiness Home (SPEC) | 🟢 **SPEC delivered — pending human review** 2026-07-07 (`docs/strategy/Sx_UI_05_TODAY_READINESS_HOME_SPEC.md` + `docs/SPRINT_Sx_UI_05_REPORT.md`). Home = **surface de décision quotidienne** (CTA unique : reprendre/démarrer/choisir/repos), Readiness V1 = **bande qualitative self-report** (jamais score médical), progress snapshot + lien, body continuity léger. Data contract **existing data only**, aucun modèle/migration/service. 10 OQ tranchées. Build split 5 sous-sprints (IA/hero → active/next → readiness → progress/body → empty/a11y). Sx_UI_04 non rouvert. |
| **Prochaine action** | **Human review** de `Sx_UI_05_TODAY_READINESS_HOME_SPEC` + confirmer OQ-05-A → OQ-05-J. **Puis ouvrir `Sb_UI_05.1 Home IA + Hero Decision Surface`** sur override explicite. `Sx_UI_06 Exercise Intelligence Presentation` future, not opened. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra. |
| Ruff budget | **534 ≤ 548** |
