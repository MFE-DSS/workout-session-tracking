...
| Sb_UI_04.1 CSS Foundation Build | 🟢 **DELIVERED 2026-07-04 — pending CI + human review** (`docs/SPRINT_Sb_UI_04_1_REPORT.md`). Premier sprint code visuel du cycle. `app/static/css/session_focus.css` : +216 lignes (tokens Auren scoped + overrides teal chirurgical désaturé + mono metrics + focus visible universel + WCAG 44×44 preserved). OQ-H/I/J/K/M tranchées V1. Bridge additif vers variables legacy (`--accent`, `--ok`, `--warn`, `--fg-dim`, `--bg`, `--border`) redéfinies localement. **Aucun template modifié. Aucun JS modifié. Aucun service métier touché. 93/93 tests session focus passent.** |
| **Prochaine action** | **Vérifier CI verte** sur push Sb_UI_04.1. **Puis human review** (comparaison visuelle baseline vs after via capture locale opérateur dans `var/visual-after/Sb_UI_04_1/`). **Puis proposition `Sb_UI_04.2 Session Focus Header & Jump Bar Structure`** (partials templates + CSS renforcement) sur override explicite. Options différables non urgentes : release tag baseline preauren, mini-gate 57 kg, cleanup warning bcrypt. |
| Ruff budget | **534 ≤ 548** |
