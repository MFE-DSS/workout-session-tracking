...
| Sb_UI_10.1 Visible Product Strings (SPIGNOS → Auren) | 🟢 **SAFE BUILD — DELIVERED LOCALEMENT (LOCAL BUILD MODE, non commité)** 2026-07-15 (`docs/SPRINT_Sb_UI_10_1_VISIBLE_PRODUCT_STRINGS_REPORT.md`). Nom produit **visible** migré SPIGNOS → **Auren** sur les 4 chaînes de `base.html` (apple-title, `<title>`, brand topbar, footer). SPIGNOS reste interne (commentaire ; 0 visible). Template-only : routes/classes/static/manifest/nav/logout intacts ; auth standalone + manifest différés (`10.3`/`10.2`). Zéro Orion, aucun asset. 14 dédiés + 50 asservis + sweep 386 verts. check_scope ISOLATED→**promu SHARED_CODE** (base.html = 37 pages). **Recommandation : GO COMMIT + CI.** |
| Sx_UI_10 Auren Visual Migration Closeout & Rebrand Readiness | 🟢 **SPEC/AUDIT/CLOSEOUT RÉDIGÉ (docs-only, commité `49fa7d3`)** 2026-07-15 (`docs/strategy/Sx_UI_10_AUREN_VISUAL_MIGRATION_CLOSEOUT_AND_READINESS_SPEC.md`). **Option A** : identité Auren Terminal actée (substantiellement livrée) + audit migration visible SPIGNOS→Auren. Canon SPIGNOS=interne / Auren=visible / Auren Terminal=identité. Gap = nom visible (base.html/welcome/login/register/science/atlas/coach_report/manifest) + assets Auren + due diligence nom/domaine. **NON-GOAL** : renommage code/repo/package/route/model/DB. Split `Sb_UI_10.1` strings → `.2` PWA+icons → `.3` auth/welcome → `.4` docs/labels → closeout. Brouillon « Orion » supprimé (0 résidu). **Premier build recommandé : `Sb_UI_10.1` Visible Product Strings** (base.html, template-only, sûr). |
