...
| Dogfood Load Hint / Substitution Coherence | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-11. Investigation dogfood : cohérence entre la charge suggérée (overload hint / placeholder) et la substitution d'exercice (le hint suit-il le substitut choisi ?). Sur override explicite. |
| **Prochaine action** | `Sb_UI_06.3` **accepté** ✅ ; cycle de dé-densification (carte exercice + Worked Area + home) livré. Prochaines pistes : **Dogfood Load Hint / Substitution Coherence** (cohérence hint↔substitution), `Sb_UI_06.4` (écrans secondaires : session done / coach report), **ou** reprise **Body Intelligence** (UI stabilisée). Autres choix (override) : `Sb_32.4` (bascule consommateurs métier). Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
