...
| **Prochaine action** | **Dogfooding terrain demain matin** (checklist §7 du closeout report). Cycle **Sx_DOGFOOD_01 CLOSED**. Décision **après le retour terrain** : si cohérence confirmée → envisager **deploy** (GO explicite) + reprise **Body Intelligence** ; si « charge faussement intelligente » remontée → build correctif ciblé ; **Option C** (unité séparée) seulement si le placeholder compact reste ambigu. Pistes hors cycle (aucune ouverte sans GO) : `Sb_UI_06.4`, `Sb_32.4`, release tag, smokes UI auth prod. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
