...
| Sb_UI_06.2 Worked Area Density Cleanup | 🟢 **DELIVERED — pending CI + human review** 2026-07-10 (`docs/SPRINT_Sb_UI_06_2_WORKED_AREA_CLEANUP_REPORT.md`). Chip de zone décoratif retiré ; body-map discret conservé ; **« À qualifier » 1× max** (rows vides masquées ; stabilisation permanente retirée) ; note raccourcie ; `data-resolution-path` conservé. Template-only ; `body_map_descriptor`/route/D1/D2 inchangés ; logging/SSR/no-JS intact. 10 sujets clivants brainstormés. 93 ciblés verts ; ruff 541 ≤ 548. En attente **GO commit** puis CI. |
| **Prochaine action** | `Sb_UI_06.2` **livré, en attente GO commit**. Après → CI 3/3, human review. Ensuite `Sb_UI_06.3` (Home : teaser readiness → pointeur + KPI = sous-ensemble), `.4` (écrans secondaires), **ou** reprise Body Intelligence après stabilisation UI. Autres choix (override) : `Sb_32.4` (bascule consommateurs métier). Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
