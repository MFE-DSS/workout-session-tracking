...
| Sb_32.2 ExerciseMuscleMapping + classify lookup/fallback | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-08. Table `ExerciseMuscleMapping` + bascule `classify_exercise` → lookup DB avec fallback substring, **prouvée non-régressive contre la baseline `.1`** (91 exercices reproduits à l'identique) avant tout basculement. Sur override explicite opérateur. |
| Sb_32.3 → .4 | ⏸️ **BLOCKED** (séquentiels, review-gated). `body_map_descriptor` · migration consommateurs (coach/body_intel/scoring). Chaque sous-sprint prouve la non-régression (classify old=new) avant le suivant. |
| **Prochaine action** | `Sb_32.1` **accepté** ✅, cycle Sx_32 **in progress**. **Ouvrir `Sb_32.2 ExerciseMuscleMapping + classify_exercise lookup/fallback`** (spec-then-build) sur override explicite opérateur, sous garde de la baseline de non-régression `.1`. **Non ouvert dans ce commit.** Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
