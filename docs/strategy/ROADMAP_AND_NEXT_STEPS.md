...
| Sx_BI_01 Body Intelligence Activation | 🟢 **SPEC + AUDIT LIVRÉS — READY FOR HUMAN DECISION** 2026-07-11 (`docs/strategy/Sx_BI_01_BODY_INTELLIGENCE_ACTIVATION_SPEC.md` + `docs/SPRINT_Sx_BI_01_BODY_INTELLIGENCE_ACTIVATION_AUDIT_REPORT.md`, docs-only). **Constat** : BI **pas un greenfield** — composer `/body/intelligence` **flag-off**, `/physique` **LIVE avec score A/B/C opaque**, fondations Sx_32 prêtes (11 zones, 91 exos mappés, Muscle vide), `body_map_descriptor` câblé. **Choix : Option A — Zone Intelligence Cards** (par zones, traçable, confidence-aware, non médical ; pas de 2e score opaque ; pas de home widget). 15 sujets clivants tranchés. Build minimal futur `Sb_BI_01.1`. **Aucun code/modèle/migration/JS/deploy/claim médical.** |
| **Prochaine action** | **Décision humaine sur Sx_BI_01** : valider l'angle Option A (Zone Intelligence Cards) → ouvrir `Sb_BI_01.1` sur **GO séparé**, ou ajuster l'angle. `Sb_BI_01.1` 🟡 READY TO BE PROPOSED, not opened. **Dogfooding terrain Sx_DOGFOOD_01** toujours prévu (checklist §7 du closeout). Pistes hors cycle (aucune ouverte sans GO) : `Sb_UI_06.4`, `Sb_32.4`, release tag, smokes UI auth prod. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31, activation `/body` Manual Profile, décision score `/physique`. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
