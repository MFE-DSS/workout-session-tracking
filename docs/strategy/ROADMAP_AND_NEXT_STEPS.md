...
| **Prochaine action** | **Vérifier CI verte** sur push Sb_UI_02b.3 (3 jobs). **Puis human review** (cohérence Home/Focus/Shell graphite/mono/ambre ; pages coque login/register/progression désormais terminal). **Puis closeout `Sx_UI_02b`** (bilan migration Home+Focus+Shell, résidu sparkline documenté) OU retour `Sb_UI_05.2` sur override. `Sx_UI_06` future. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py` (service). |
| Ruff budget | **534 ≤ 548** |
