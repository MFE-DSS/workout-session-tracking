...
| Sx_UI_07.1 Progress Surface Auren Readability | 🟢 **DELIVERED — pending GO commit + CI + human review** 2026-07-11 (`docs/SPRINT_Sx_UI_07_1_PROGRESS_AUREN_READABILITY_REPORT.md`). **Option A pure** (template-only sur `/progress`) : lede utile, section « Rythme récent », titres timelines sobres, note « Lecture indicative ». Tous les blocs conservés ; **aucun service touché** (`kpis.py`/`timeline.py`/`weekly_loop.py`/`pages.py` non ouverts) ; aucune valeur/calcul changé ; aucun JS/score/lien BI-physique/couleur/rebrand. 11 tests dédiés + broad sweep 232 verts. Labels KPI « sessions » **conservés** (2 assertions textuelles de `test_kpis.py` hors périmètre y sont asservies → décision : rester dans le périmètre `progress.html` seul). check_scope=ISOLATED. |
