...
| Sb_ASSET_01.2 Body Zone Taxonomy & Mapping Contract | 🟢 **CODE COMPLETE — CI PENDING — HUMAN REVIEW PENDING** 2026-07-19 (baseline `7da5334` ; `docs/SPRINT_Sb_ASSET_01_2_BODY_ZONE_TAXONOMY_MAPPING_REPORT.md`). 2ᵉ/dernier build du socle `Sx_ASSET_01`. Contrat sémantique versionné (Layer A, 0 géométrie, 0 runtime) : `AUREN_BODY_ZONE_TAXONOMY.md` + `source/bodymap/auren_bodymap_mapping.yaml` (YAML JSON-compatible, `json` stdlib, 0 dépendance) + README source. Parité testée : zones ↔ `ZONE_LABELS`, mapping ↔ `_WA_ZONE_TO_REGION`, descriptor 11 codes/`unknown`. `unknown` = état de qualification (pas une 12ᵉ zone). `BODYMAP COMPACT MACROS ARE NOT RADAR_AXES` (séparation testée ; `RADAR_AXES`/`RADAR_AXIS_ORDER` intouchés). IDs SVG figés. 2 dettes 01.1 résolues (owner nuancé via `ip_ownership_status`, garde binaire → garde évolutive allowlistée). 29 + 23 tests dédiés + broad sweep 273 verts. 0 SVG/PNG/master/géométrie/dépendance/`app/**`/Custom/analytics ; services + template byte-identiques. check_scope ISOLATED ; ruff clean, budget 543 ≤ 548, spec PASS. Sx_UI CLOSED ; ASSET INTEGRATION GATE BLOCKED. **Prochaine étape : `GO VALIDATE — Sb_ASSET_01.2`** (non commencé). |
