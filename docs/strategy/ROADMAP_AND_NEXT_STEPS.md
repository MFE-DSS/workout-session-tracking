...
| Sb_32.4 | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-09. Migration consommateurs (coach/body_intel/scoring) vers lookup DB + descriptor. Review-gated, prouve la non-régression (classify old=new) avant bascule. Sur override explicite opérateur. |
| Sx_32 closeout | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-09. Acter la fondation (BodyZone/Muscle/Mapping/descriptor) + 1er consommateur UI livrés ; planifier/différer la bascule métier. Alternative à `Sb_32.4`. Sur override explicite opérateur. |
| **Prochaine action** | `Sb_32.next.worked-area-descriptor-ui` **accepté** ✅. Cycle Sx_32 **in progress** (`.1`→`.3` + UI + scope-guard acceptés). Arbitrage : ouvrir `Sb_32.4` (bascule consommateurs métier vers lookup DB + descriptor) **ou** `Sx_32 closeout` (clôturer sur la fondation + 1er consommateur UI, différer la bascule). Sur override explicite. Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
