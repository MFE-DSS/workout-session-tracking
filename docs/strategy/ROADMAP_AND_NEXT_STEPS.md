...
| Sx_TRANSFORM_01 App Transformation Consolidation | 🟢 **SPEC + AUDIT LIVRÉS — READY FOR HUMAN DECISION** 2026-07-11 (`docs/strategy/Sx_TRANSFORM_01_APP_TRANSFORMATION_CONSOLIDATION_SPEC.md` + `docs/SPRINT_Sx_TRANSFORM_01_APP_TRANSFORMATION_CORPUS_AUDIT_REPORT.md`, docs-only). **Document maître** (Option A) consolidant le corpus. Vocabulaire fixé (SPIGNOS repo / Auren produit / Auren Terminal identité ; « Spinos » inexistant). Direction active **Auren Terminal dark/mono/amber** (white clinical déjà tranché Sx_UI_02b). Architecture figée (SSR/Jinja, **no React**, no-JS fallback, PWA-first). Priorités réalignées (mode séance › Home › cohérence charge › BI zones › Progress › PWA › Rebrand) + principes actionnables (silence plutôt que faux poids, pas de score opaque, confidence, non-médical, ne pas re-densifier). Contradictions résolues, interdits explicités. **Aucun code/rebrand/deploy.** En attente GO commit. |
| **Prochaine action** | **GO commit Sx_TRANSFORM_01** (spec/audit docs-only) attendu. Puis **décider quel build ouvrir sur GO séparé** : (a) `Sb_BI_01.1` **Zone Intelligence Cards** 🟡 READY TO BE PROPOSED — angle Option A validé, **non bloqué** par le corpus (11/11 zones primaires, 0 unknown) ; (b) **corpus improvement** (zones secondaires / stabilisateurs) — amélioration **non bloquante**, sur GO séparé. **Dogfooding terrain Sx_DOGFOOD_01** toujours pending. Body Intelligence build / deploy **deferred until explicit GO**. Pistes hors cycle : `Sb_UI_06.4`, `Sb_32.4`, `Sx_UI_07` (Progress), `Sx_UI_08` (PWA), release tag, smokes UI auth prod. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31, activation `/body` Manual Profile, décision score `/physique`, rebrand `Sx_UI_10`. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
