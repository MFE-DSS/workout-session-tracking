...
| Sx_UI_10 Auren Visual Migration Closeout & Rebrand Readiness | 🟢 **SPEC/AUDIT/CLOSEOUT RÉDIGÉ (docs-only, non commité)** 2026-07-15 (`docs/strategy/Sx_UI_10_AUREN_VISUAL_MIGRATION_CLOSEOUT_AND_READINESS_SPEC.md`). **Option A** : identité Auren Terminal actée (substantiellement livrée) + audit migration visible SPIGNOS→Auren. Canon SPIGNOS=interne / Auren=visible / Auren Terminal=identité. Gap = nom visible (base.html/welcome/login/register/science/atlas/coach_report/manifest) + assets Auren + due diligence nom/domaine. **NON-GOAL** : renommage code/repo/package/route/model/DB. Split `Sb_UI_10.1` strings → `.2` PWA+icons → `.3` auth/welcome → `.4` docs/labels → closeout. Brouillon « Orion » supprimé (0 résidu). **Premier build recommandé : `Sb_UI_10.1` Visible Product Strings** (base.html, template-only, sûr). |
