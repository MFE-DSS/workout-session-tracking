...
| **Prochaine action** | **GO `Sx_UI_11_SCREENSHOT_REGRESSION_SPEC` SPEC ONLY.** Sur override opérateur explicite. Docs-only strict. Baseline screenshots obligatoire **avant** tout code visuel Sx_UI_04. Sx_UI_04 (premier sprint visuel avec code) reste **BLOQUÉ** jusqu'à : Sx_UI_11 baseline disponible + OQ résiduelles Sx_UI_02 tranchées (OQ-H hex, OQ-I sans, OQ-J mono, OQ-K scale, OQ-M naming) + OQ-R Progression sub-nav. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
