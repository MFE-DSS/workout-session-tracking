...
| Sb_UI_11.1 Screenshot Tooling Build | 🟢 **DELIVERED 2026-07-02 — pending human review + CI complète** (`docs/SPRINT_Sb_UI_11_1_SCREENSHOT_TOOLING_BUILD_REPORT.md`). Matrice P0/P1/P2 (16/14/6 screenshots), CLI Playwright avec dry-run + strict-p0 + anti-secret hard, 92 tests unitaires (canary env leak validé), extra dep `[baseline]` optionnel, `.gitignore` mis à jour. **Aucun app code touché.** Aucun screenshot capturé côté sprint. Aucun Chromium installé en CI. Ce push **déclenche CI complète** (~22-25 min compute) — comportement voulu. |
| **Prochaine action** | **1) Vérifier CI verte** sur le push Sb_UI_11.1. **2) Human review** du sprint. **3) Capture P0 locale opérateur** : `pip install -e '.[baseline]'` + `python -m playwright install chromium` + `.env.baseline` fixture + `python scripts/visual_baseline_capture.py --priority P0 --viewport all --out-dir var/visual-baseline`. **4) Upload artefact ou release tag** `baseline-preauren-YYYYMMDD`. **Alors** ouvrir `Sb_UI_04.1_CSS_FOUNDATION_BUILD` sur override explicite. **Sb_UI_04.1 reste BLOQUÉ** tant que baseline P0 non capturée localement. Aucune dérogation baseline n'est accordée. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
