...
| Sb_UI_04.2 Header & Jump Bar Structure | 🟢 **DELIVERED 2026-07-04 — pending CI + human review** (`docs/SPRINT_Sb_UI_04_2_REPORT.md`). Refonte structurelle : header avec wrappers Auren (`.session-focus__header-main`, `-title-row`, `-kicker`, `-meta`, `-progress-value`) + `.back` intégré + truncate ellipsis titre + progression mono/tabular. Jump bar avec `aria-current="location"` (WAI-ARIA sémantiquement propre, remplace "step"/"false") + non-color cues via CSS pseudo `::before` (`●`/`✓`/`◐`/`○`/`–`/`↔`). 13 nouveaux tests + 2 tests patchés. **106/106 tests session focus verts.** Zones interdites intactes. Aucun asset ajouté. |
| **Prochaine action** | **Vérifier CI verte** sur push Sb_UI_04.2. **Puis human review** (comparaison visuelle after-capture opérateur dans `var/visual-after/Sb_UI_04_2/`). **Puis proposition `Sb_UI_04.3 Exercise Cards + Set Logging Visual Refinement`** sur override explicite. Options différables non urgentes : release tag baseline preauren, mini-gate 57 kg, cleanup warning bcrypt. |
| Ruff budget | **534 ≤ 548** |
