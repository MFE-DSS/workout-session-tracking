...
| **Sb_ASSET_02.1-fix Review Preview Rendering Fix** | 🟢 **PREVIEW FIXED — CI PENDING — RE-REVIEW REQUIRED** 2026-07-21 (baseline `64ab789` ; `docs/SPRINT_Sb_ASSET_02_1_FIX_PREVIEW_RENDERING_REPORT.md`). Corrige l'unique motif de rejet de `Sb_ASSET_02.1` : preview via **CSS mask** (géométrie dans les 10 SVG canoniques, `mask-image` 10 URL distinctes, couleur par `background-color: currentColor`) → graphite sur clair, **ambre sur graphite**, 16/20/24 px, 0 géométrie inline, 0 JS/CDN. **Non-régression** : 10 SVG/registre/LICENSES/manifest/provenance byte-identiques ; garde inchangée ; 45 tests verts ; test preview renforcé. **0** modif SVG/licence/gouvernance/`app/**`/Custom. `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO VALIDATE — Sb_ASSET_02.1`** (re-review). |
| **Sb_ASSET_02.1 Vendored Icon Subset & License Intake** | 🔴 **CODE COMPLETE — CI GREEN (`29749856878` 3/3 sur `eafede6`) — HUMAN REVIEW REJECTED (preview only) → PREVIEW CORRIGÉE PAR `Sb_ASSET_02.1-fix`** 2026-07-20 (`docs/SPRINT_Sb_ASSET_02_1_VENDOR_ICON_SUBSET_LICENSE_INTAKE_HUMAN_REVIEW_REPORT.md` ; commit `804b08c`). **Assets/licence/gouvernance VÉRIFIÉS** (upstream 10/10 byte-for-byte, licence MIT byte-identique `b740a1d4…`, 0 approved/0 verified, garde effective par test négatif, 0 app change). **Rejet UNIQUEMENT sur la surface de revue (§14)** : preview via `<img>` → `currentColor` non transmis → icônes noires invisibles sur graphite (`DARK PREVIEW DOES NOT REPRESENT A USABLE REVIEW SURFACE`). Corrigée par `Sb_ASSET_02.1-fix` (ci-dessus, CSS mask). 10 icônes NOT AUTHORIZED FOR APP INTEGRATION ; `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO VALIDATE — Sb_ASSET_02.1`** (re-review après correctif preview). |
| **Sx_ASSET_02 Functional Iconography Selection** (2ᵉ cycle `Sx_ASSET`) | 🟢 **SPEC COMMITTED** 2026-07-20 (`fe97adc` ; `docs/strategy/Sx_ASSET_02_FUNCTIONAL_ICONOGRAPHY_SELECTION_SPEC.md` + rapport + `docs/research/AUREN_ICON_VENDOR_DUE_DILIGENCE.md`). SEMANTICS BEFORE ICONS. Due diligence datée : Tabler v3.45.0 `975920ff…` MIT (primaire) ; Health Icons `891ace7a…` CC0 assets/MIT code (NOT REQUIRED FOR P0). P0 = 10 Tabler ; 0 gap custom → `Sb_ASSET_02.2 NOT REQUIRED`. Rend `ICON INTAKE GATE: READY FOR Sb_ASSET_02.1` ; `ASSET INTEGRATION GATE: BLOCKED`. **Build : `Sb_ASSET_02.1` (ci-dessus).** |
