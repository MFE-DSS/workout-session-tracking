...
- **Nouveau track — Morphology Program** : `Sx_MORPHO_PROGRAM_01` (architecture, **SPEC-ONLY**) = ✅
  **MERGED + VERIFIED** (`8d1ce98`). File de build (chacun sur GO explicite, dans l'ordre des dépendances) :
  ...
  4. **`Sb_MARTIN_PROGRAM_01`** — fixture dogfood privée Martin + programme dérivé = ⏳ **PR GREEN / MERGE PENDING** (base `a6d5d75`, branche `sb/martin-program-01` ; dérivation **pure/privée** via le générateur : fixture `tests/fixtures/dogfood/martin_program.py` + doc dogfood + 11 tests + broad sweep 122 ; programme `mpg1-391c65154b3ed546`, 8 intentions Full Body Morphotype, **3 remplis / 5 warnings de couverture, 0 fabrication** ; 0 persistance/DB/Custom/publication/EKB/donnée globale ; **FINDING** : les priorités morphotype de Martin — latéraux/postérieurs/mollets — ne sont **pas couvertes** par `exercise_properties`, d'où le besoin d'un build de **couverture `exercise_properties`** avant l'item 5 ; merge = **GO humain**) ;
