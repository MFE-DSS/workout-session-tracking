...
| Sb_UI_02b.2 Focus Re-skin | ⏸️ **next candidate, not opened** (post-Sb_UI_02b.1 CI + review). Migrer `session_focus.css` (cockpit Sx_UI_04) → graphite/mono/ambre, invariants structurels intacts. |
| Sb_UI_02b.3 · Sb_UI_05.2 | ⏸️ **BLOCKED / DEFERRED**. .3 hardening + shell/nav + baseline re-capturée · Sb_UI_05.2 différé derrière la migration visuelle. |
...
| **Prochaine action** | **Vérifier CI verte** sur push Sb_OPS.sonar-java21 (les 3 jobs, dont SonarCloud sous Java 21). **Si CI verte** → CI repo débloquée → **human review Sb_UI_02b.1** (comparaison `var/visual-after/Sb_UI_05_1/home-*` teal vs `Sb_UI_02b_1/home-*` terminal). **Puis `Sb_UI_02b.2 Focus Re-skin`** sur override. `Sb_UI_05.2` et `Sx_UI_06` différés. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra. |
| Ruff budget | **534 ≤ 548** |
