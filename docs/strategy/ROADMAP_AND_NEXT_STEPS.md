...
| **Prochaine action** | **GO Sx_UI_03 App Shell Navigation SPEC ONLY.** Ouverture sur override opérateur. Docs-only strict. Aucun build UI ne peut démarrer avant : Sx_UI_01 ✅ + Sx_UI_02 ✅ + Sx_UI_03 accepté + Sx_UI_11 baseline disponible. `Sx_UI_04` (Session Focus Reskin, premier sprint autorisé à toucher du code) reste **BLOQUÉ**. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
