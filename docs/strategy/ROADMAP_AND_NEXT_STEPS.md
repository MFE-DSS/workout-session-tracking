...
| **Prochaine action** | ✅ **Sx_UI_06 (dé-densification UI) SPEC ACCEPTED** 2026-07-09 (`docs/strategy/Sx_UI_06_INFO_DENSITY_DEDUP_SPEC.md`) : règle « une info = un seul endroit », D1 (charge → console) + D2 (cible → case) + OQ-A→E tranchées. Origine : point d'attention opérateur (surcharge informationnelle). **Ouvrir `Sb_UI_06.1`** (carte d'exercice, D1+D2, ROI max) sur GO. Autres choix (override séparé) : `Sb_32.4` (bascule consommateurs métier), cycle Body Intelligence. Options différables : release tag, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
