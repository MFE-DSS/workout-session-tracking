...
| Sb_OPS.ci-efficiency (parallélisation tests) | ✅ **MERGED + CANONICAL CI GREEN** 2026-07-30 (build `626fc55`, **PR #42 MERGED**, merge `9d98f82` via `--merge` **sans `--admin`** — gate `CLEAN` ; `docs/SPRINT_Sb_OPS_CI_EFFICIENCY_REPORT.md`). **Phase 1** du virage « développement efficace » : `pytest-xdist` + job CI `test` en `pytest -n auto --dist worksteal` (coverage.xml / 8 QA scripts / jobs `lint`+`sonar` préservés) ; `.check-policy.json` tier `shared_code` **allégé** (full sweep local non systématique, `broad_sweep_scoped` conservé, CI parallélisée = filet) ; `CLAUDE.md` §1 doctrine mise à jour (commande de référence xdist, `ci_infra` prouve sur CI réelle). **Gain CI : job `test` ~37 → ~11 min** (PR #42 `11:06`, canonique `30520718331` `11:41`), **couverture intacte** (coverage.xml produite+consommée sous xdist, PR + trunk), dogfood local **834 s → 298.72 s ≈ 2.8×**, **2677 passed / 0 `serial`**. check_scope **CI_INFRA**, budget 543 ≤ 548. **Zéro `app/`, zéro migration.** Leviers 3 (CI deux vitesses) + 4 (sélection par impact / testmon) **DEFERRED**. Évolue `Sb_OPS.scope-guard`. |
| Sb_OPS.sonar-coverage-fix (fix couverture SonarCloud) | ✅ **MERGED + CANONICAL CI GREEN + REPORT CLOSED + CLEANED** 2026-08-05 (build `62df735`, **PR #47 MERGED**, merge `cc3978e` via `--merge` **sans `--admin`** — gate `CLEAN` ; closeout `219313c` ; `docs/SPRINT_Sb_OPS_SONAR_COVERAGE_FIX_REPORT.md`). Répare le reporting Sonar **cassé à `0.0 %`** (double-préfixe `sed` sous le scanner Java 21) : `relative_files=true` (`pyproject`) + retrait du step `sed` (`ci.yml`). **Coverage `0.0 % → 91.1 %`** (PR #47 + trunk `cc3978e`, run `30996238572` 3/3). `SonarCloud Code Analysis` (rouge à chaque PR de code avant) désormais **vert** ; **fin du bypass `--admin` structurel** — validé sur WIZARD_06 PR #44 (`new_coverage 100 %`, gate `CLEAN`). Full sweep **2700 passed**, check_scope CI_INFRA. Zéro `app/`/migration. |
| Sb_OPS.agent-autonomy-01 (protocole de livraison agentique) | 🟢 **MERGED + CANONICAL CI GREEN** 2026-08-07 (base `190fa8c`, **PR #51 MERGED**, merge **`a55618e`** via `--merge` **sans `--admin`** — gate `CLEAN` ; **CI canonique `31188046923` 3/3 GREEN**, coverage 91.1 %, Sonar delta 0 ; `docs/strategy/AGENTIC_DELIVERY_PROTOCOL.md` + `CLAUDE.md §4` + registry/roadmap). **Protocole ACTIF sur le canonique** ; cette livraison en est le 1ᵉʳ exemple (`GO BUILD` → autonome jusqu'à PR GREEN ; `GO MERGE` → autonome jusqu'à CLOSED + CLEANED). Formalise une boucle spec-driven **autonome** pour réduire les micro-GO. **`GO BUILD`** → autonome jusqu'à `PR GREEN / MERGE PENDING` (branche/worktree · patch **scopé** · checks `check_scope` · fix **in-scope** · commit · push · PR · récup CI ×1 · fix CI/Sonar in-scope). **`GO MERGE`** → autonome jusqu'à `CLOSED + CLEANED` (merge **méthode approuvée** · vérif CI canonique + Sonar · closeout docs · push docs-only · cleanup). **Garde-fous durs préservés** — jamais en autonomie : merge/squash/`--admin`/suppression branche·worktree/changement de périmètre/affaiblir gates/secrets/`AGENTS.md`/worktree non lié/amender silencieusement une spec. **Arrêts durs** : conflit spec · ambiguïté migration · destruction données · sécurité · secret · credential externe · thread exigeant du code · CI rouge hors périmètre · décision de merge · dérive de périmètre. Règles spéciales MIGRATION / ASSET-anatomie / docs-only. **Docs + `CLAUDE.md`, zéro `app/`/test/migration/comportement produit.** Livré **via PR** (pas de merge en autonomie — GO humain). Évolue `Sb_OPS.scope-guard`. |
| Sb_OPS.sonar-hygiene-p1 (safe batch : split assertions test) | 🟢 **MERGED + CANONICAL CI GREEN** 2026-08-07 (base `038d194`, **PR #55 MERGED**, merge `45ead13` via `--merge` **sans `--admin`** — gate `CLEAN` ; CI canonique `31253319877` 3/3, coverage 91.1 %, Sonar delta 0 ; `docs/SPRINT_Sb_OPS_SONAR_HYGIENE_P1_REPORT.md` ; sous protocole agentique). **1ʳᵉ tentative (PR #54, dédup `S1192` squads) abandonnée** — cascade `S8415` + `new_coverage` 21.1 % (toucher des lignes d'error-handling réveille les smells latents + expose leur non-couverture) → fermée non mergée, branche/worktree supprimés. **Retarget `python:S9073`** (split `assert A and B`) **test-only** : 6 assertions dans 2 tests Body Intelligence (`test_bi01_zone_drill_detail` + `_intelligence_cards`), faits indépendants, **intention préservée, aucune assertion affaiblie, aucun impact coverage**. **22 tests BI passés**, check_scope **ISOLATED**, budget 543 ≤ 548. **Leçon** : cibler des lignes **déjà couvertes & sans smell latent** (tests), pas le code d'erreur. **Prochaine action** : merge PR (GO humain) puis closeout ; lots S9073 suivants au besoin. |
...
| Sb_UI_09.3 Contrast Guard | ✅ **HUMAN REVIEW ACCEPTED** 2026-07-19 (build `1281e85`, CI [`29692113635`](https://github.com/MFE-DSS/workout-session-tracking/actions/runs/29692113635) 3/3 premier coup ; `docs/SPRINT_Sb_UI_09_3_CONTRAST_GUARD_HUMAN_REVIEW_REPORT.md`). Revue : `1281e85` ancêtre du HEAD `e88865c` (closeout Custom, 0 drift, app.css byte-identique) ; test lit les vraies valeurs + WCAG stdlib ; **garde effectif prouvé par mutation** (fg-dim 5.31→3.38 échouerait) ; ratios tous ≥ AA (fg-dim min 5.31, accent 6.79–7.74) ; aucune correction CSS (déjà AA) ; test only ; 8 tests verts, 0 réorienté ; CI 3/3 ; sweep revue 428 verts. WCAG 1.4.3 (AA). **`Sx_UI_09` = 3 lots acceptés → prêt pour closeout.** **Prochaine action : `GO CLOSEOUT — Sx_UI_09`** (non commencé). — 3ᵉ et dernier lot de `Sx_UI_09`. |
| Sb_UI_09.3 Contrast Guard | 🟢 **CODE COMPLETE — CI PENDING — HUMAN REVIEW PENDING** 2026-07-19 (baseline `bf5998e` ; `docs/SPRINT_Sb_UI_09_3_CONTRAST_GUARD_REPORT.md`). 3ᵉ et dernier lot de `Sx_UI_09`. Test de garde (pas de correction) verrouillant le contraste WCAG AA des tokens : `test_contrast_guard.py` lit les vraies valeurs d'app.css, calcule les ratios (stdlib, 0 dépendance) et asserte ≥ 4.5 (texte) / ≥ 3.0 (UI). Ratios mesurés tous ≥ AA (--fg-dim min 5.31 sur --surface-2 > 4.5 ; --accent 6.79–7.74 ; --on-accent 8.14). Aucune correction CSS (tokens déjà AA). Test only : 0 fichier applicatif touché. WCAG 1.4.3 (AA). 8 tests dédiés, 0 réorienté. ruff clean, budget 543 ≤ 548, spec PASS, check_scope ISOLATED. **Les 3 lots Sx_UI_09 livrés** (09.1/09.2/09.3). **Prochaine étape : `GO VALIDATE — Sb_UI_09.3`** (non commencé ; puis closeout Sx_UI_09). — 2ᵉ lot de `Sx_UI_09`. |
| Sb_UI_09.2 Form-Errors ARIA | ✅ **HUMAN REVIEW ACCEPTED** 2026-07-18 (build `ac6cf20`, CI [`29655710750`](https://github.com/MFE-DSS/workout-session-tracking/actions/runs/29655710750) 3/3 premier coup ; `docs/SPRINT_Sb_UI_09_2_FORM_ERRORS_ARIA_HUMAN_REVIEW_REPORT.md`). Revue : role=alert sur 8 conteneurs (7 templates) + id/aria-describedby conditionnel login/register ; 0 aria-invalid (choix honnête) ; rendu HTTP réel prouvé (login 401 sert role=alert) ; contrat POST/champs/couleurs inchangés ; no-JS ; template only ; 11 tests dédiés verts, 0 réorienté ; CI 3/3 ; sweep revue 327 verts. WCAG 4.1.3 (AA) + 3.3.1. **Prochaine action : `GO BUILD — Sb_UI_09.3 Contrast Guard`** (non commencé). — 2ᵉ lot de `Sx_UI_09`. Erreurs de formulaire annoncées : `role="alert"` sur les 8 conteneurs `.integrity-errors` des 7 templates (login/register/reset_password/forgot_password/contact/password_change/export) + `id`/`aria-describedby` conditionnel sur `<form>` login/register. Pas d'`aria-invalid` (message global, champ fautif non identifié → serait faux ; choix honnête). SSR/no-JS ; contrat POST/champs/labels/couleurs inchangés. Template only ; 0 router/service/model/migration/manifest/asset/JS/contrat/Custom. WCAG 2.2 4.1.3 (AA) + 3.3.1. 11 tests dédiés, 0 réorienté. ruff clean, budget 543 ≤ 548, spec PASS, check_scope ISOLATED. **Prochaine étape : `GO VALIDATE — Sb_UI_09.2`** (non commencé). Reste : `09.3` contrast guard, closeout. |
| Sb_UI_09.1 Reduced-Motion Global | ✅ **HUMAN REVIEW ACCEPTED** 2026-07-17 (build `a2c753a`, CI [`29586630426`](https://github.com/MFE-DSS/workout-session-tracking/actions/runs/29586630426) 3/3 premier coup ; `docs/SPRINT_Sb_UI_09_1_REDUCED_MOTION_GLOBAL_HUMAN_REVIEW_REPORT.md`). Revue : `a2c753a` ancêtre du HEAD `a1fe5a6` (merges Custom PR #25/#26, 0 drift) ; bloc reduced-motion **purement additif** (0 sélecteur modifié), motion only, blocs scopés préservés, 0 hex, 9 tests dédiés verts, CI 3/3, sweep revue 811 verts. **Prochaine action : `GO BUILD — Sb_UI_09.2 Form-Errors ARIA`** (non commencé). — 1er lot de `Sx_UI_09`. Bloc universel `@media (prefers-reduced-motion: reduce) { *, ::before, ::after { animation/transition-duration 0.01ms !important } }` en fin d'app.css — neutralise les transitions/animations décoratives (9 transitions auditées, toutes décoratives ; 0 animation active). Pattern WCAG 2.2 de référence, robuste aux ajouts futurs, blocs scopés (session_focus/body_intelligence) préservés. Motion only : 0 perte d'info, 0 display:none, 0 changement contenu/couleur/layout. CSS only (additif) ; 0 template/route/service/model/migration/manifest/asset/JS/contrat/Custom. 9 tests dédiés, 0 réorienté. Local : 9 + broad sweep 808 verts ; ruff clean, budget 543 ≤ 548, spec PASS, check_scope ISOLATED. **Prochaine étape : `GO VALIDATE — Sb_UI_09.1`** (non commencé). Suite : `09.2` form-errors ARIA, `09.3` contrast guard, closeout. |
| **Muscle Focus Runtime & Enrichment (`Sb_ASSET_04.1-P0` → `04.2-P1`)** | 🟢 **P0 CLOSED / DELIVERED · P1 MERGED + CANONICAL CI GREEN** 2026-08-07. **P0 (`Sb_ASSET_04.1-P0`)** : runtime des 3 plaques Muscle Focus **SSR / no-JS** sur `/science` — **MERGED** (PR #49, merge `ae10737`, CI canonique `31161250110` 3/3, closeout `1ea97bf` + cleanup branches/worktrees). **Dogfood Martin = FOUNDATION ACCEPTED** ; enrichissement visuel/produit **requis**, classé **MAJOR NEXT-ITERATION** (ni dette mineure, ni rejet de la fondation). **P0 TECHNICAL FOUNDATION: CLOSED / DELIVERED.** **P1 (`Sb_ASSET_04.2`)** : spec d'enrichissement `docs/strategy/Sb_ASSET_04_2_MUSCLE_FOCUS_ENRICHMENT_P1_SPEC.md` — **template + CSS, no-JS** à fort gain de valeur perçue : layout/rythme (plaques agrandies par la mise en page, jamais le `viewBox`) + cartes de synthèse par région + divulgation progressive `<details>` « ce que ça montre / ne montre pas » + lisibilité des libellés (contraste AA). **Choix `B+C+D+A` ; `E` (nouvelles plaques/régions) → P2+ ; `F` (génération/overlay médical-like) interdit.** **Garde-fous P1 verrouillés** : pas de % activation · pas d'EMG · pas de % recrutement · **non médical** · provenance **BodyParts3D** conservée · **pas d'anatomie finale IA** · **contrat additif uniquement** · SSR/no-JS préféré · pas de réécriture géométrie/score/modèle/migration sauf approbation séparée. **Politique image** : références visuelles seulement, aucun asset volumineux committé sans décision `docs/assets` séparée. **P1 BUILT** (`SPRINT_Sb_ASSET_04_2_MUSCLE_FOCUS_ENRICHMENT_P1_REPORT.md`, branche `sb/asset-04-2-muscle-focus-enrichment-p1`, base `831a38f`, sous protocole agentique) : 3 cartes région + divulgation progressive `<details>` + lisibilité ; plaques agrandies par le **layout** (`viewBox`/paths inchangés) ; toggle no-JS préservé ; « activation » → « effort ». **12 tests** (8 P0 + 4 P1, tautologie toggle corrigée) + broad sweep ciblé **54** (dont contrast guard) ; check_scope **ISOLATED**. **P1 MERGED** — **PR #52**, merge `47e7c0e` via `--merge` **sans `--admin`** (gate CLEAN) ; **CI canonique `31201147891` 3/3 GREEN**, coverage 91.1 %, Sonar delta 0 ; branche + worktree nettoyés. **1ᵉʳ build de code livré de bout en bout sous le protocole agentique** (`GO BUILD` → PR GREEN → `GO MERGE` → CLOSED+CLEANED). Dette a11y différée (toggle épaules avant le `<h3>`, `<legend>` nomme déjà la région). `E`/`F` restent hors P1. |
| Sx_ASSET_03B Muscle Focus Technical Surface System | 🟢 **SPEC COMMITTED (docs-only)** 2026-07-24 (`docs/SPRINT_Sx_ASSET_03B_MUSCLE_FOCUS_TECHNICAL_SURFACE_SYSTEM_REPORT.md` + `docs/strategy/Sx_ASSET_03B_MUSCLE_FOCUS_TECHNICAL_SURFACE_SYSTEM_SPEC.md` + `docs/research/AUREN_MUSCLE_FOCUS_REFERENCE_RESEARCH.md` + `docs/production/muscle-focus/` ×3 ; base `b1f0b63` ; worktree `work/sx-asset-03b-muscle-focus-system`). **Sous-cycle ADDITIF et stratégique de `Sx_ASSET`** — **ne remplace pas** le bodymap global, **redéfinit la couche visuelle premium**. Correction de cible : la vraie valeur = « **comment** ce muscle s'insère/se contracte, quels exercices le sollicitent », pas « où ça tape ». **Système à 3 niveaux ancrés sur les 11 zones immuables** : **N1 = Global BodyMap CONSERVÉ** (navigation/synthèse/score) · **N2 = 8 Regional Focus Plates** (une par macro, `legs` éclatée en quads/posterior/calves) · **N3 = 11 Muscle Focus / Exercise Mechanics Plates** (faisceaux/insertions/fonctions/exercices ; 9 muscle-heads + 2 grouped-honest). **Aucune 12ᵉ zone** (faisceaux = géométrie Layer B, jamais des codes). Contrat d'IDs de plaque distinct de l'API `zone-<code>` ; états projetés sur les 5 états figés ; mobile vs desktop. Direction clean-view + technical-overlay, profondeur par le trait, ancrage osseux ; **5 cas d'exigence** (pectoraux sans « poumons » · deltoïdes 3 faisceaux ancrés · postérieur zoom bassin · lats/upper_back largeur vs épaisseur · core corset). **Stratégie de sources source-reuse-first** (Servier + OpenStax 1ʳᵉ éd. + PD = géométrie ; **BodyParts3D CC BY 4.0 / Z-Anatomy = validation seulement** ; MuscleWiki/BioDigital/OpenStax 2ᵉ éd. = exclus ; IA = plan B/C borné, déclaré). **Contradiction de licence arbitrée** par croisement adversarial (relevé « CC BY-SA » = miroir GitHub figé → écarté ; officiel DBCLS = **CC BY 4.0**). **Amendement de gouvernance requis au build** : fibres/insertions autorisées **uniquement sur la surface plaque dédiée** (le global reste régi par `AUREN_STYLE_RULES §5`). Queue `Sb_ASSET_03B.1→4` gatée. **7 fichiers créés + 3 mis à jour, 100 % `docs/**`** ; check_scope DOCS ; spec PASS ; commit 100 % docs → `CI: SKIPPED — DOCS-ONLY / PATHS-IGNORE`. **0** asset/SVG/`app/**`/tests/scripts/`data/**`/migrations/Custom ; `Sx_UI`/`Sx_ASSET_01`/`02`/`03` non rouverts. `MUSCLE FOCUS PLATES: CONCEPTUALLY DEFINED / NOT PRODUCED` · `GLOBAL BODYMAP: RETAINED` · `SOURCE STRATEGY: DEFINED` · `RUNTIME INTEGRATION: NOT STARTED` · `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action (non ouverte)** : `GO BUILD — Sb_ASSET_03B.1 Muscle Focus System Blueprint & Plate Template`. |
| Sb_ASSET_03B.1 Muscle Focus System Blueprint & Plate Contract | 🟢 **CLOSED — CANONICAL DELIVERY COMPLETE** (merge `805b8a9` / **PR #34 MERGED** 2026-07-26 ; blueprint `95f47b3` ; **1er parent réel `c48714b6`** = closeout SCORING_03 — merge sans conflit, SCORING_03 préservé ; **PR CI 3/3 verte** run `30193520498` ; aucun run post-merge — docs-only paths-ignore ; closeout `docs/SPRINT_Sb_ASSET_03B_1_FINAL_CLOSEOUT_REPORT.md`). **`Sb_ASSET_03B.2: READY TO OPEN / NOT STARTED`**. — *build docs ci-dessous (historique).* 🟢 **BLUEPRINT COMPLETE / PLATE CONTRACT LOCKED (docs-only)** 2026-07-24 (`docs/strategy/Sb_ASSET_03B_1_MUSCLE_FOCUS_SYSTEM_BLUEPRINT_SPEC.md` + `docs/SPRINT_Sb_ASSET_03B_1_MUSCLE_FOCUS_SYSTEM_BLUEPRINT_REPORT.md` + `docs/production/muscle-focus/` ×7 ; base `ff9541a` ; worktree `work/sb-asset-03b-1-muscle-focus-blueprint`). Vision `Sx_ASSET_03B` → **système exécutable sur le papier, 0 géométrie**. **5 axes parallèles** (Contract&IDs · P0 Visual Blueprint · Source **revalidation web réelle** · UX/Mobile/A11y · **Adversarial Review**) + synthèse arbitrée unique. Fige : **ID Contract v0.1.0** (namespace `auren-plate-*` **disjoint de l'API `zone-*`**, anti-collision DOM ; registre `part-*` ; 7 prédicats) · descriptor schema (9 invariants) · view/crop contract (front/back partout, lateral/section N3-only, `viewbox_local` gaté) · **7 blueprints P0 exacts** (chest N2+N3, shoulders N2 + delt_lat + delt_post, posterior N2+N3 ; clean+caption+**lien exercice zone-liste**) · overlay/a11y/mobile (**caption = miroir de l'overlay**, module 7 → `overlay-shortening`) · **source ledger revalidé — 4 corrections** (NLM Visible Human **terms-based/validation**, **clause anti-ingestion IA OpenStax**, Gray's/atlas **PD conditionnel**, Servier **images-only**) · governance amendment `§5bis` borné aux surfaces plaque (**master global inchangé**), **spécifié / enactment gaté** + guard test. **8 findings adversariaux résolus.** **9 créés + 6 modifiés = 15 fichiers, 100 % `docs/**`** ; check_scope DOCS ; spec PASS ; `git diff --check` propre ; commit docs → `CI: SKIPPED — DOCS-ONLY / PATHS-IGNORE`. **0** géométrie/SVG/`viewBox` chiffré/`app/**`/`design/**` (amendement non enacté)/tests/scripts/migrations. `PLATE GEOMETRY: NOT PRODUCED` · `GOVERNANCE AMENDMENT: SPECIFIED / NOT YET ENACTED` · `SOURCE STRATEGY: REVALIDATED` · `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action (non ouverte)** : `GO BUILD — Sb_ASSET_03B.2 Regional Plate Production & Intake`. |
| Sb_ASSET_03B.2 P0 Regional Plate Production Package | 🟢 **OPEN — PHASE 1 PRODUCTION PACKAGE READY / GEOMETRY PENDING (AWAITING OPERATOR TOOLCHAIN) (docs-only, lot a)** 2026-07-27 (base `c70bdb0` ; worktree `work/sb-asset-03b-2-p0-regional-plates` ; `docs/strategy/Sb_ASSET_03B_2_P0_REGIONAL_PLATE_PRODUCTION_SPEC.md` + rapport + `docs/production/muscle-focus/AUREN_MUSCLE_FOCUS_P0_REGIONAL_REVIEW_PROTOCOL.md`). **Décision de provenance** : géométrie anatomique **non productible par l'IA** (§6 interdit l'IA-anatomie ; `ai_usage: NONE` ; provenance de source acquise requise) → **produite par le toolchain opérateur** (Blender/Potrace/Inkscape sur Servier/OpenStax acquis), comme le master `Sb_ASSET_03.1`. **2 lots** : (a) package de production (ce commit — briefs 3 plaques chest/shoulders/posterior, contrat SVG/descripteur/registry/guard/preview, budgets, clarification « crop du master » = repère seul + géométrie redessinée + `GLOBAL BODYMAP: UNCHANGED`, sources, protocole revue par plaque) ; (b) intake atomique sur GO (**`§5bis` enactment + guard test + descripteurs/registry/manifest/preview à provenance réelle** sur la géométrie livrée). `§5bis` **non enacté ce lot** (couplé au guard+géométrie). **3 créés + 4 modifiés, 100 % `docs/**`** ; check_scope DOCS ; spec PASS. **0** géométrie/SVG/`design/auren/**`/tests/`app/**`/Custom. PR #35 (Custom wizard) + dependabot préservées. `REGIONAL PLATE GEOMETRY: NOT PRODUCED (this lot)` · `GLOBAL BODYMAP: UNCHANGED` · `HUMAN ANATOMICAL REVIEW: REQUIRED / NOT STARTED` · `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action (sur GO)** : `GO OPERATOR` (géométrie) puis `GO INTAKE — Sb_ASSET_03B.2` (via PR). |
| Sb_ASSET_03B.2R BodyParts3D Source-Contract Reset | 🟢 **SOURCE DOCTRINE RESET LOCALLY / UNCOMMITTED (docs-only + policy tests)** 2026-07-29 (base `bc1a42cc` ; worktree `work/sb-asset-03b-2r-bodyparts3d-source-reset`). Socle géométrique P0 réinitialisé **Servier → BodyParts3D 4.0 (DBCLS officiel, CC BY 4.0)** après confirmation produit humaine de Martin (scope method+governance-only, `confirmed_at_utc: 2026-07-29T11:44:19Z`). **Couverture P0 prouvée** — résolution **exact-FMA**, **35 représentations**, 0 conflit, **0 match artériel** (un premier proof sous-chaîne matchait des artères → corrigé). **Segmentation** : pectoral = mesh source **entier** / partitions functional-visual ; **deltoïde = source-segmenté** (clavicular/acromial/spinal → antérieur/latéral/postérieur) ; posterior N2 `muscle-heads` / N3 `grouped-honest` inchangés. **Doctrine 3 niveaux** (A déterministe BP3D · B sculpting humain conditionnel non-automatisé · C Open3DModel CC BY-SA séparé **non autorisé**) ; IA générative anatomie **interdite** ; **double gate** revue (produit Martin + anatomique qualifié `REQUIRED_PENDING`). **10 docs** (2 créés + 8 mis à jour, historique préservé) + `tests/test_auren_muscle_focus_source_policy.py`. **0 binaire/OBJ/géométrie dans Git** ; catalogue 3 432 concepts **hors Git**. `P0 GEOMETRY: NOT PRODUCED` · `QUALIFIED ANATOMICAL REVIEW: REQUIRED_PENDING` · `ASSET INTEGRATION GATE: BLOCKED` · `RUNTIME: BLOCKED` · `§5bis: NOT ENACTED`. **Prochaine action (GO séparé)** : `GO BUILD` géométrie (Blender déterministe) — revue indépendante. |
| Sb_ASSET_03B.2R-C2 Synthetic Single-Family Multi-Agent Review Council | ✅ **MERGED + CANONICAL CI GREEN** 2026-08-09 (**PR #46 MERGED**, merge `d73af7d` via `--merge --match-head-commit` sans squash/`--admin` ; **CI canonique `31310301874` 3/3 GREEN**, Sonar delta 0, 3/3 threads Gitar résolus ; harnais salvagé après triage `Sb_OPS` — `GO RESOLVE PR #46` ; base d'origine `9d98f82d` ; branche `work/sb-asset-03b-2r-synthetic-multimodel-review`). Géométrie P0 chest/shoulders/posterior (produite hors Git par le toolchain opérateur BodyParts3D) soumise à un **review interne synthétique** après **report du dispatch réviseur externe C1** (ZIP `e1dc96e3…` préservé, non envoyé, gate externe **non passée**). **Harness réutilisable Promptfoo 0.120.27 (MIT)** versionné (`tools/evals/muscle_focus/` : config, 6 prompts, 2 schémas, 4 rubriques, 9 scripts, 22 tests) — **0 candidat/binaire/credential dans Git**. **Conseil aveugle** 5 juges + arbitre/région ; providers indisponibles → **run via sous-agents Claude Code (`SINGLE_FAMILY_MULTI_AGENT`)**, config promptfoo rejouable plus tard. **45/45 juges valides · calibration 9/9 crit + 12/12 (PASS) · agrégation déterministe ordre-indépendante.** chest **87.6 HIGH ACCEPTED_WITH_CONSTRAINTS** (contour sternal + z-layering origine/insertion) · shoulders **87.6 HIGH ACCEPTED_INTERNAL** · posterior **85.2 HIGH ACCEPTED_WITH_CONSTRAINTS** (tendons distaux ischio) ; 0 veto ; partition chest `REVIEW_PARTITION_UNRESOLVED / NOT ACCEPTED`. Reco globale `SYNTHETIC_ACCEPTED_WITH_CONSTRAINTS` (résultat historique — a alimenté le C3 freeze → intake D1 → runtime 04.1, déjà livrés). `PROFESSIONAL ANATOMICAL REVIEW: NOT PERFORMED / NOT CLAIMED` · `EXTERNAL DISPATCH: DEFERRED BY OWNER` · `RUNTIME/INTAKE/§5bis: BLOCKED` (ce sprint) · `CANDIDATES: UNCHANGED`. **Résolution `Sb_OPS` (GO RESOLVE PR #46, 2026-08-09)** : le **harnais réutilisable** (eval only, 26 tests, 0 binaire/candidat/credential, self-contained) a été salvagé (rebasé additivement + 3 threads Gitar corrigés) et **MERGÉ** sur la canonique (`d73af7d`) — versionné pour les revues de plaques futures ; **aucun runtime/intake/§5bis**. |
| Sb_ASSET_01.1 Governance Scaffold & Provenance Registry | 🟢 **CODE COMPLETE — CI GREEN (`29697874021` 3/3) — HUMAN REVIEW ACCEPTED** 2026-07-19 (`SPRINT_Sb_ASSET_01_1_GOVERNANCE_SCAFFOLD_HUMAN_REVIEW_REPORT.md` ; commit `4603551` ; 2 dettes cosmétiques → `Sb_ASSET_01.2` : nuancer `owner` + temporalité garde binaire ; gate BLOCKED). Baseline `6167485` ; `docs/SPRINT_Sb_ASSET_01_1_GOVERNANCE_SCAFFOLD_REPORT.md`). 1er build de `Sx_ASSET`. Scaffold `design/auren/` (GOVERNANCE BEFORE ASSETS, 0 asset produit) : README + manifest (16 champs, 8 statuts, 0 entrée `approved`, valeurs UNKNOWN distinctes) + provenance (18 champs, repository-authored, NONE tiers) + style rules (non médical, 11 zones) + intake checklist + LICENSES/README (0 licence tierce). Assets runtime (BodyMap prototype, PWA, icônes inline) référencés sans copie/modification, classés provisional/prototype. 21 tests dédiés (stdlib only) : 0 binaire sous design/auren, 0 master, 0 faux `approved`, assets runtime en place. 0 asset/SVG/PNG/YAML/JSON/licence/dépendance/`app/**`/Custom/métier. check_scope ISOLATED ; ruff clean, budget 543 ≤ 548, spec PASS ; broad sweep 263 verts. Sx_UI CLOSED ; ASSET INTEGRATION GATE BLOCKED. **Prochaine étape : `GO VALIDATE — Sb_ASSET_01.1`** (non commencé). |
| Sb_ASSET_01.2 Body Zone Taxonomy & Mapping Contract | 🟢 **CODE COMPLETE — CI GREEN (`29702926887` 3/3) — HUMAN REVIEW ACCEPTED** 2026-07-20 (`docs/SPRINT_Sb_ASSET_01_2_BODY_ZONE_TAXONOMY_MAPPING_HUMAN_REVIEW_REPORT.md` ; commit `86aba63` ; parités revérifiées indépendamment, garde évolutive prouvée par test négatif). Baseline `7da5334` ; `docs/SPRINT_Sb_ASSET_01_2_BODY_ZONE_TAXONOMY_MAPPING_REPORT.md`). 2ᵉ/dernier build du socle `Sx_ASSET_01`. Contrat sémantique versionné (Layer A, 0 géométrie, 0 runtime) : `AUREN_BODY_ZONE_TAXONOMY.md` + `source/bodymap/auren_bodymap_mapping.yaml` (YAML JSON-compatible, `json` stdlib, 0 dépendance) + README source. Parité testée : zones ↔ `ZONE_LABELS`, mapping ↔ `_WA_ZONE_TO_REGION`, descriptor 11 codes/`unknown`. `unknown` = état de qualification (pas une 12ᵉ zone). `BODYMAP COMPACT MACROS ARE NOT RADAR_AXES` (séparation testée ; `RADAR_AXES`/`RADAR_AXIS_ORDER` intouchés). IDs SVG figés. 2 dettes 01.1 résolues (owner nuancé via `ip_ownership_status`, garde binaire → garde évolutive allowlistée). 29 + 23 tests dédiés + broad sweep 273 verts. 0 SVG/PNG/master/géométrie/dépendance/`app/**`/Custom/analytics ; services + template byte-identiques. check_scope ISOLATED ; ruff clean, budget 543 ≤ 548, spec PASS. Sx_UI CLOSED ; ASSET INTEGRATION GATE BLOCKED. **Socle `Sx_ASSET_01` implémenté (01.1 + 01.2 acceptés) → COMPLETE / READY FOR CLOSEOUT.** **Prochaine étape : `GO CLOSEOUT — Sx_ASSET_01`** (non commencé). |
| **Sb_ASSET_02.1-fix Review Preview Rendering Fix** | 🟢 **PREVIEW FIXED — CI GREEN (`29815584673` 3/3 sur `8342d99`) — HUMAN RE-REVIEW ACCEPTED** 2026-07-21 (`docs/SPRINT_Sb_ASSET_02_1_VENDOR_ICON_SUBSET_LICENSE_INTAKE_HUMAN_REREVIEW_REPORT.md`). Preview via **CSS mask** (10 SVG canoniques, `mask-image` 10 URL distinctes, `background-color: currentColor`) — **rendu vérifié en NAVIGATEUR RÉEL** (Chrome headless local) : graphite sur clair, **ambre sur graphite** (419 px ambre / 0 px noir), matrice 10×3×2, mobile 360 utilisable. Immutabilité VÉRIFIÉE (10 SVG/licence/registre/manifest/provenance/garde byte-identiques) ; test preview renforcé. `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO CLOSEOUT — Sx_ASSET_02`**. |
| **Sb_ASSET_02.1 Vendored Icon Subset & License Intake** | 🟢 **CODE COMPLETE — CI GREEN (`29749856878` 3/3) — HUMAN REVIEW ACCEPTED** 2026-07-21 (après correctif preview `Sb_ASSET_02.1-fix` + re-review). **ICON SOURCE INTAKE: ACCEPTED FOR DESIGN SOURCE** ; assets/licence/provenance HUMAN REVIEW VERIFIED (upstream 10/10 byte-for-byte, licence MIT `b740a1d4…`) ; LEGAL CLEARANCE NOT CLAIMED. 10 icônes NOT AUTHORIZED FOR APP INTEGRATION ; `ASSET INTEGRATION GATE: BLOCKED`. **`Sx_ASSET_02 implementation: COMPLETE / READY FOR CLOSEOUT`.** *(Rejet initial `64ab789` sur la seule preview — corrigé ; rapport de rejet conservé.)* **Prochaine action : `GO CLOSEOUT — Sx_ASSET_02`**. |
| **Sx_ASSET_03B Muscle Focus Technical Surface System** (sous-cycle additif · spec) | 🟢 **SPEC COMMITTED (docs-only)** 2026-07-24 (`docs/strategy/Sx_ASSET_03B_MUSCLE_FOCUS_TECHNICAL_SURFACE_SYSTEM_SPEC.md` + rapport + `docs/research/AUREN_MUSCLE_FOCUS_REFERENCE_RESEARCH.md` + `docs/production/muscle-focus/` ×3 ; base `b1f0b63`). **Correction stratégique de cible** : le bodymap global était trop « corps entier » pour la surface héro ; la vraie valeur = « **comment** ce muscle s'insère/se contracte, quels exercices le sollicitent », pas « où ça tape ». **Additif — ne remplace pas** le bodymap global, il **redéfinit la couche premium**. **Système 3 niveaux ancrés sur les 11 zones immuables** : N1 Global BodyMap **conservé** (navigation/synthèse/score) · N2 **8 Regional Focus Plates** (une par macro, `legs` éclatée) · N3 **11 Muscle Focus / Exercise Mechanics Plates** (faisceaux/insertions/exercices ; 9 muscle-heads + 2 grouped-honest). **Aucune 12ᵉ zone** (heads = géométrie Layer B). Direction clean-view + technical-overlay, profondeur par le trait, ancrage osseux ; **5 cas d'exigence** (pectoraux sans « poumons » · deltoïdes 3 faisceaux ancrés · postérieur zoom bassin · lats/upper_back largeur vs épaisseur · core corset). **Source-reuse-first** (Servier + OpenStax 1ʳᵉ éd. + PD = géométrie ; BodyParts3D CC BY 4.0/Z-Anatomy = validation ; MuscleWiki/BioDigital/OpenStax 2ᵉ éd. = exclus ; IA plan B/C borné). **Contradiction de licence arbitrée** (BodyParts3D CC BY 4.0 officiel ; le « CC BY-SA » venait du miroir GitHub figé). Amendement de gouvernance §5 requis au build (fibres/insertions **uniquement sur la plaque dédiée**). Queue `Sb_ASSET_03B.1→4` gatée. **7 créés + 3 mis à jour, 100 % docs.** `MUSCLE FOCUS PLATES: CONCEPTUALLY DEFINED / NOT PRODUCED` ; `GLOBAL BODYMAP: RETAINED` ; `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO BUILD — Sb_ASSET_03B.1 Muscle Focus System Blueprint & Plate Template`** (non ouvert). |
| **Sb_ASSET_03.1-fix Reproducibility Closure & Package Reissue** | 🟢 **COMPLETE — REPRODUCIBILITY GAP CLOSED** 2026-07-23 (docs-only côté Git ; `docs/SPRINT_Sb_ASSET_03_1_REPRODUCIBILITY_FIX_REPORT.md` + `docs/production/bodymap/SB_ASSET_03_1_REPRODUCIBILITY_CLOSURE.md`). Ferme `INCOMPLETE EXECUTABLE BUILD GRAPH` relevé par l'intake. **Producteur manquant scripté** (`extract_servier_regions.py`, **sélecteurs exacts 117/157 récupérés**, non reconstruits) → **parité byte-identique** (`dc04a017…`, `fe01dc94…`). **14/14 scripts relocalisés** (0 `/Users/`, 0 réseau, 0 `shell=True`) via `pipeline_context.py`. **Graphe complet** (`NO ORPHAN INPUTS`), **entrypoint** `run_pipeline.py`. **Deux replays clean-room** (dont racine espace+Unicode) → master `dbb57db3…` et compact `8024fd4c…` **exactement reproduits**, 32 previews. **Package v2** auto-descriptif (**62 = 61 + 1**) et **déterministe** (deux racines → `f45e0dbf…`, 1 449 359 o). Package v1 (`098d1b42…`) et preuves d'intake **conservés**. `MASTER`/`COMPACT` **inchangés**. **Prochaine action : `GO RESUME INTAKE — Sb_ASSET_03.2 from §11 using package v2`.** |
| **Sb_ASSET_03.2 BodyMap Prototype Technical Validation & Design-Source Intake** | 🟢 **TECHNICAL VALIDATION PASSED (package v2) / HUMAN REVIEW PENDING** 2026-07-23 (`docs/SPRINT_Sb_ASSET_03_2_BODYMAP_PROTOTYPE_TECHNICAL_VALIDATION_REPORT.md` + spec + `production/bodymap/SB_ASSET_03_2_*` ×5). **Reprise avec package v2** après `Sb_ASSET_03.1-fix`. Validation indépendante : identité `f45e0dbf…` (62 entrées), manifeste embarqué `62=61+1` (61 re-hachés), **replay clean-room byte-identique** (master `dbb57db3…`, compact `8024fd4c…`, package `f45e0dbf…`, 32 previews) dans une racine espace+Unicode vierge, SVG 40/40+66/66, Chrome+Inkscape concordent, provenance CC BY 4.0 double sans contamination. **Intake design source** : master + compact + registre YAML + notices + surface HTML + garde `tests/test_auren_bodymap_master.py` (22 tests, 7 négatifs) ; évolutions gouvernées, 97 tests Auren verts. **0 `app/**`, 0 binaire committé.** `MASTER: TECHNICALLY VALIDATED / NOT YET HUMAN APPROVED` ; `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO HUMAN REVIEW — Sb_ASSET_03.2 Auren BodyMap Design Source`.** — *tentative 1 (bloquée) ci-dessous.* |
| **Sb_ASSET_03.2 BodyMap Prototype Technical Validation (tentative 1)** | ⛔ **BLOCKED — NON-REPRODUCIBLE OUTPUT** 2026-07-23 (`docs/SPRINT_Sb_ASSET_03_2_BODYMAP_PROTOTYPE_TECHNICAL_VALIDATION_REPORT.md`). Intégrité v1 PASS ; **master 66/66** ; **un seul gate échoue** : reproductibilité — producteur Servier absent du package v1. Rien copié dans `design/`, rien committé. → corrigé par `Sb_ASSET_03.1-fix`. |
| **Sb_ASSET_03.1 Open Anatomy Source Acquisition & BodyMap Derivation Prototype** | 🟢 **BUILD COMPLETE / MIXED-SOURCE PROTOTYPE READY FOR TECHNICAL INTAKE** 2026-07-23 (docs-only côté Git ; `docs/SPRINT_Sb_ASSET_03_1_OPEN_ANATOMY_SOURCE_DERIVATION_REPORT.md` + `docs/production/bodymap/SB_ASSET_03_1_*` ×6). **Prototype réellement produit hors Git** depuis deux sources **CC BY 4.0**. Blender 5.2.0 LTS · Potrace 1.16 · Inkscape 1.4.4, **verbes intégrés seuls**, 0 dépendance, `AI_USAGE: NONE`. Licence BP3D revalidée le jour même ; 3 artefacts officiels hachés ; archives saines (2 234 + 1 258 OBJ, 0 traversée/symlink) ; **package ISA retenu et justifié**. **Correction** : le vrai index de mapping est `isa_element_parts.txt` (`FJ####`), pas `isa_parts_list_e.txt` (`BP####`, recouvrement 0). **55 maillages**, 0 FMA inventé, base = **FMA7163 skin**. Topologie sur maillages utilisés : **0 non-manifold**, **0 face dégénérée**, **112 paires en intersection** → intersections peau/muscle **confirmées**, non-manifold `FMA7163` **non reproduit**, sans généralisation. **Servier `EDITABLE VECTOR`** (3 361 chemins natifs) → dérivation raster inutile ; `lats`/`core` par masque monochrome + **transformation uniforme chiffrée**. **Master 40/40** (59 620 o) · **compact 41/41 à 8 615 o ≤ 12 Ko**. Revue multi-sources **11 PASS / 0 BLOCKED** (OpenStax 1ʳᵉ éd. + Sobotta 1909) ; revue professionnelle **NON revendiquée**. **32 previews** ; package hashé **14 495 063 o** `098d1b42…`. **0 contamination**. `BODYMAP MASTER: NOT YET APPROVED` ; `ATTRIBUTION SURFACE: NOT YET IMPLEMENTED` ; `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO INTAKE — Sb_ASSET_03.2`.** — *état initial ci-dessous.* |
| **Sb_ASSET_03.1 Open Anatomy Source Acquisition & BodyMap Derivation Prototype** | 🟢 **SPEC READY / BUILD NOT STARTED** 2026-07-23 (docs-only ; `docs/strategy/Sb_ASSET_03_1_OPEN_ANATOMY_SOURCE_DERIVATION_SPEC.md`). Chemin retenu pour le master BodyMap : **dérivation depuis sources ouvertes** au lieu d'une commande externe. Pipeline 13 étapes **non exécuté** (téléchargement borné depuis l'archive **officielle DBCLS** · hashes/provenance · mapping OBJ→zones · structures superficielles seules · scène Blender reproductible · caméras orthographiques face/dos · rendu monochrome par zone · vectorisation Inkscape/Potrace · nettoyage · regroupement **11 zones** · viewBox `0 0 240 200` + **14 IDs** + **0 zone-unknown** · croisement Servier/AnatomyTOOL/OpenStax 1ʳᵉ éd. · 32 previews). Gate révisé : **cohérence multi-sources REQUISE**, revue professionnelle **NON revendiquée/optionnelle**, illustrateur nommé **plus un préalable**. Contamination ShareAlike/NC = **échec de build**. Qualité qualifiée par niveau de preuve : `UPSTREAM QUALITY WARNING` **officiel** cité verbatim (erreurs déclarées, parties d'origine artistique ou déformées → **pas une vérité anatomique**) mais `SPECIFIC MESH DEFECTS: NOT YET VERIFIED` (observation d'un dépôt tiers → `BUILD INSPECTION CHECKS`). **2 points non tranchés volontairement** : choix d'archive/version (IS-A vs PART-OF, 4.0 vs 3.0) et nature vectorielle du pack Servier `.pptx`. 0 archive/SVG/image/`app/**`/test/Custom. **Prochaine action : `GO BUILD — Sb_ASSET_03.1`.** |
| **Sx_ASSET_03 — amendement SOURCE-REUSE-FIRST** (3ᵉ cycle `Sx_ASSET`) | 🟠 **AMENDED — SOURCE-REUSE-FIRST** 2026-07-23 (docs-only ; `docs/SPRINT_Sx_ASSET_03_AMENDMENT_SOURCE_REUSE_FIRST_REPORT.md` + `docs/research/AUREN_BODYMAP_OPEN_SOURCE_REUSE_STRATEGY.md` créés ; spec + due diligence + 4 docs `production/bodymap/` amendés). **Correction juridique majeure sur source officielle** : **BodyParts3D = CC BY 4.0** (DBCLS, maj **2025-02-27**), **pas** CC BY-SA 2.1 Japan — le relevé du 2026-07-22 venait d'un **miroir GitHub figé en 2011**. Sans ShareAlike, **la dérivation vers un master propriétaire est licite** sous **attribution obligatoire** → BodyParts3D devient `PRIMARY DERIVATION SOURCE`. Hiérarchie : Servier **CC BY 4.0** (contrôle 2D) · AnatomyTOOL par ressource · **OpenStax 1ʳᵉ éd. CC BY / 2e éd. NC exclue** · Z-Anatomy (BY-SA + NC) référence · Wikimedia `Muscles front and back.svg` (BY-SA, dérivé d'OpenStax) **prototype jetable**. Règles anti-contamination dures ; **un prototype ShareAlike ne se « blanchit » pas**. Nuance conservée : Auren possède sa contribution, **pas** les données ; « Auren owns the master » reste faux sans qualification. Le §1 périmé de la due diligence est **conservé comme preuve historique**. Nouvelle dette `ATTRIBUTION SURFACE: NOT YET IMPLEMENTED`. `OPERATOR_ASSET_03.1` → **repli**. `BODYMAP MASTER: NOT YET PRODUCED` ; `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO BUILD — Sb_ASSET_03.1`.** — *état antérieur ci-dessous.* |
| **Sx_ASSET_03 BodyMap Human Production Package** (3ᵉ cycle `Sx_ASSET`) | 🟢 **SPEC COMMITTED** 2026-07-22 (commit **`66d18d47a1f1f9a75556bede8de45f9a94daa055`**, parent `062ee92`, poussé sur le canonique ; commit 100 % `docs/**` → `CI: SKIPPED — DOCS-ONLY / PATHS-IGNORE` ; docs-only ; `docs/strategy/Sx_ASSET_03_BODYMAP_HUMAN_PRODUCTION_PACKAGE_SPEC.md` + rapport + `docs/research/AUREN_BODYMAP_REFERENCE_DUE_DILIGENCE.md` + `docs/production/bodymap/` ×6 ; baseline `cf41188`). Transforme le contrat sémantique BodyMap (immuable) en **package de production humaine exécutable** — **master NON produit**. Brief illustrateur (`male_neutral_v1` P0, biomécanique non médical), **contrat SVG tranché** (viewBox `0 0 240 200`, 14 IDs figés, `<g>`/zone, 0 zone-unknown), exigences PI (checklist, pas un contrat), protocole revue (anatomique ≠ médical + produit/mobile + 32 previews bornées), manifest template (0 approved), contrat `Sb_ASSET_03.2` préparé. **Due diligence datée** : BodyParts3D **CC BY-SA** = référence spatiale uniquement (dérivation écartée) ; AnatomyTOOL par ressource. IA = moodboard uniquement. 12 fichiers 100% docs ; 0 SVG/design/tests/app/asset/Custom. `BODYMAP MASTER: NOT YET PRODUCED` ; `OPERATOR_ASSET_03.1: PACKAGE READY / NOT STARTED` ; `PROFESSIONAL LEGAL CLEARANCE: NOT CLAIMED` ; `ASSET INTEGRATION GATE: BLOCKED`. **Prochaine action : `GO OPERATOR — OPERATOR_ASSET_03.1 Human BodyMap Master Production`** (production humaine / externe, pas un build logiciel). |
| **Sx_ASSET_02 Functional Iconography Selection & Vendor Intake** (2ᵉ cycle `Sx_ASSET`) | 🔒 **CLOSED / HUMAN REVIEW COMPLETE** 2026-07-21 (`docs/SPRINT_Sx_ASSET_02_FINAL_CLOSEOUT_REPORT.md` ; closeout `9ca1e58`). Cycle clos : sélection iconographique COMPLETE + intake tiers **Tabler P0 (10 SVG)** ACCEPTED (chaîne `fe97adc`→`804b08c`→`64ab789` rejet→`8342d99` fix→`9ca1e58` re-review). Licence MIT byte-identique, pins VERIFIED, provenance/manifest 10 entrées (0 approved), garde allowlistée. Rejet initial limité à la preview → corrigé (CSS mask, **rendu vérifié en navigateur réel** : ambre sur graphite, mobile 360). CI intake descendant `29749856878` 3/3 + correctif `29815584673` 3/3. **OPEN IMPLEMENTATION DEBT: NONE.** Health Icons/custom/P1/BodyMap/runtime NON produits. **ICON SOURCE INTAKE: ACCEPTED FOR DESIGN SOURCE** ; 10 icônes NOT AUTHORIZED FOR APP INTEGRATION ; LEGAL CLEARANCE NOT CLAIMED ; `ASSET INTEGRATION GATE: BLOCKED` ; `Sb_ASSET_02.2 NOT REQUIRED`. Programme global Sx_ASSET non fermé ; `Sx_ASSET_03` NOT OPENED. **Prochaine action : `GO SPEC — Sx_ASSET_03 — BodyMap Human Production Package`.** — *historique spec/build ci-dessous.* |
| **Sx_ASSET_02 (spec) Functional Iconography Selection** (2ᵉ cycle `Sx_ASSET`) | 🟢 **SPEC COMMITTED** 2026-07-20 (`fe97adc` ; `docs/strategy/Sx_ASSET_02_FUNCTIONAL_ICONOGRAPHY_SELECTION_SPEC.md` + rapport + `docs/research/AUREN_ICON_VENDOR_DUE_DILIGENCE.md`). SEMANTICS BEFORE ICONS. Due diligence datée : Tabler v3.45.0 `975920ff…` MIT (primaire) ; Health Icons `891ace7a…` CC0 assets/MIT code (NOT REQUIRED FOR P0). P0 = 10 Tabler ; 0 gap custom → `Sb_ASSET_02.2 NOT REQUIRED`. Rend `ICON INTAKE GATE: READY FOR Sb_ASSET_02.1` ; `ASSET INTEGRATION GATE: BLOCKED`. **Build : `Sb_ASSET_02.1` (ci-dessus).** |
| **Sx_ASSET — Auren Proprietary Visual Asset System** (programme indépendant) | 🔒 **`Sx_ASSET_01` CLOSED / HUMAN REVIEW COMPLETE** 2026-07-20 (`docs/SPRINT_Sx_ASSET_01_FINAL_CLOSEOUT_REPORT.md` ; closeout `6345f5a`). Socle gouvernance + contrat sémantique **complet** (01.1 ACCEPTED `7da5334`/CI `29697874021` 3/3 + 01.2 ACCEPTED `1fbbb31`/CI `29702926887` 3/3 ; chaîne `6167485`→`1fbbb31`). **GRAPHICAL ASSET PRODUCTION NOT STARTED** (0 asset produit). **OPEN DEBT: NONE** (2 dettes 01.1 résolues). `app/` byte-identique tout le cycle. **`ASSET INTEGRATION GATE: BLOCKED`**. Gates ouverts : `Sx_ASSET_02` NOT OPENED, `Sx_ASSET_03`/`OPERATOR_03.1` HUMAN PRODUCTION PENDING, clearance nom EXTERNAL OPEN, `04`/`04.1` BLOCKED BY GATE. Sx_UI CLOSED. **Prochaine action : `GO SPEC — Sx_ASSET_02 Functional Iconography Selection`** (non commencé). — *historique spec ci-dessous.* |
| **Sx_ASSET_01 (spec)** (OPTIONAL FUTURE PROGRAM / OPENED BY OPERATOR GO) | ✅ **`Sx_ASSET_01` SPEC RÉDIGÉE — AUREN ASSET PROGRAM: PRODUCTION QUEUE READY** 2026-07-19 (docs-only ; `docs/strategy/Sx_ASSET_01_AUREN_VISUAL_ASSET_SYSTEM_SPEC.md` + `docs/SPRINT_Sx_ASSET_01_AUREN_VISUAL_ASSET_SYSTEM_REPORT.md` + `docs/strategy/AUREN_ASSET_PROGRAM_ROADMAP.md`). **Programme indépendant de `Sx_UI` (CLOSED, non rouvert).** Système d'assets propriétaire : taxonomie **11 zones figée** (= `ZONE_LABELS` réel, 0 semantic contract drift) + 6 macros + 4 layers (métier/géométrie/présentation/surface) + IDs-API + contrat SVG (viewBox 24, currentColor) + pipeline SVGO/resvg production-only (0 dépendance runtime) + provenance SPDX + non médical + budgets + manifest (15 champs/8 statuts). BodyMap (SVG inline Sb_BODYMAP_01.1) + icônes PWA (Sb_UI_10.2) audités comme prototype/provisoires (non supprimés). Nom Auren = working name ; brand-bearing = PROVISIONAL UNTIL PROFESSIONAL CLEARANCE. **`ASSET INTEGRATION GATE: BLOCKED`** (15 approbations humain/anatomique/juridique/mobile) — `PRODUCTION QUEUE READY` n'autorise PAS l'intégration app. Queue : `01.1` scaffold → `01.2` taxonomie → `02` iconographie → `02.1`/`.2` vendored+custom → `03` BodyMap package → `OPERATOR_03.1` master humain → `03.2` intake → `04` slots → `04.1` intégration → `05` closeout. Docs-only, 0 asset/SVG/PNG/Custom. **Prochain build : `GO BUILD — Sb_ASSET_01.1 Governance Scaffold & Provenance Registry`** (non commencé). |
| **Sx_UI (programme global)** Auren UI Transformation | ✅ **INTERNAL TRANSFORMATION PROGRAM: CLOSED / HUMAN REVIEW COMPLETE** 2026-07-19 (`docs/SPRINT_Sx_UI_GLOBAL_FINAL_CLOSEOUT_REPORT.md`). Réconciliation finale `Sx_UI_01→12`. Cycles clos (02b/03/04/07/09/10-interne ; 02 SUPERSEDED, 05 ABSORBED par 06.3, 06.4/05R/08.3 NOT REQUIRED/DEFERRED). Identité Auren Terminal + rebrand visible + shell app-like + Focus Mode + Home décisionnel + dé-densification/readability + PWA + a11y WCAG AA. Non-goals préservés (0 renommage repo/package/route/model, no-JS, SPIGNOS interne, Orion absent). **Aucun sprint UI applicatif obligatoire restant.** **GATES EXTERNES OUVERTS** : (A) `Sb_UI_11.3` baseline = OPERATOR CAPTURE PENDING · (B) nom/domaine Auren = BLOCKED FOR PROFESSIONAL CLEARANCE · (C) `Sb_SESSION_UX_01.5` dogfood = FIELD TEST READY · (D) assets Auren = futur optionnel. Le closeout ne prétend PAS baseline acceptée / nom libre / dogfood fait. **Queue post-closeout** : opérateur (11.3, dogfood), externe (clearance CPI), futur (assets) — non-UI-applicatif. |
| Sb_UI_11.3 Final Auren Baseline Refresh | 🟡 **BASELINE REFRESH READY / OPERATOR CAPTURE PENDING** 2026-07-19 (docs-only ; `docs/SPRINT_Sb_UI_11_3_FINAL_AUREN_BASELINE_REFRESH_PROTOCOL.md`). Protocole de refresh baseline visuelle post-Auren/post-shell (la baseline P0 du 2026-07-04 est obsolète). Matrice : 8 slugs P0 × 2 viewports (mobile 360×640 / desktop 1440×900) via `visual_baseline_capture.py --priority P0 --viewport all` + surfaces shell nouvelles (bottom nav, rail, topbar secondaire, skip link, indicateur session active, form-errors). Compte fixture local, storage `.gitignore` (0 PNG committé). **Capture = action OPÉRATEUR LOCALE** (agent ne peut pas : pas de Playwright ; PNG non versionnés). Checklist inspection Auren fournie. 0 code touché. **Prochaine action (opérateur)** : capture locale → rapport de capture → `Sx_UI` Global Final Closeout. |
| Sx_UI_09 Accessibility & Motion | ✅ **CYCLE CLOSED 2026-07-19** (`docs/SPRINT_Sx_UI_09_FINAL_CLOSEOUT_REPORT.md`) — spec rédigée 2026-07-17 (Option A, WCAG 2.2 AA) + **3 lots livrés/acceptés** : `Sb_UI_09.1` reduced-motion (`a2c753a`/CI `29586630426`/review `ef3b6a1`), `Sb_UI_09.2` form-errors ARIA (`ac6cf20`/CI `29655710750`/review `d80c8a5`), `Sb_UI_09.3` contrast guard (`1281e85`/CI `29692113635`/review `64a7435`), chacun **CI 3/3** premier coup. Socle WCAG AA : reduced-motion global · form-errors annoncées (role=alert, rendu réel prouvé, pas de faux aria-invalid) · contraste tokens verrouillé par test (déjà AA, hypothèse Sx_UI_12 infirmée). Non-goals préservés (AAA/JS/nouvelle couleur/charts/BodyMap/auth refonte ; non médical). Acquis a11y préservés. **Ferme la dette a11y transverse de Sx_UI_12.** **Prochaine piste** : queue `Sx_UI_12` — `Sb_UI_11.3` Final Baseline puis `Sx_UI` Global Final Closeout. |
...
- **`Sb_CUSTOM_PROGRAM_PERSISTENCE_05 — QA/Quotas Hardening` : ✅ MERGED + CANONICAL
  CI GREEN** 2026-07-19 (build `f9a62f8`, **PR #27 MERGED**, merge `bf5998e`, CI PR
  `29658159984` 3/3 · CI canonique `29687387565` 3/3)
  (`docs/SPRINT_Sb_CUSTOM_PROGRAM_PERSISTENCE_05_HARDENING_REPORT.md` + appendice,
  branche `sb/custom-program-persistence-05-hardening`, base `a1fe5a6`).
  **Zéro migration** (modèle touché sans schéma, prouvé par les 4 checks QA — head
  ...
  51 sentinelles (64/64) ; check_scope MIGRATION ; full sweep 2351 passed (unique
  failed = garde-fou arbre-sale pré-commit, vert post-commit). Quality gate externe
...
- **`Sb_CUSTOM_PROGRAM_EKB_02 — Canonical Exercise Knowledge Base JSON` : ✅ MERGED +
  ...
  **`EKB_03` (checks complets) = FIRST NEXT / NOT OPENED · `EKB_04`, `SCORING_01`,
  `WIZARD_*` NOT OPENED.**
- **`Sb_CUSTOM_PROGRAM_EKB_03 — EKB Classifiability QA` : ✅ MERGED + CANONICAL CI
  GREEN** 2026-07-21 (build `94ac158`, **PR #30 MERGED**, merge `a6be9c4`, CI PR 4/4 ·
  CI canonique `29832528309` 3/3 — la nouvelle étape `EKB classifiability QA` présente
  et verte dans le pipeline du trunk)
  (`docs/SPRINT_Sb_CUSTOM_PROGRAM_EKB_03_CLASSIFIABILITY_QA_REPORT.md` + appendice,
  branche `sb/custom-program-ekb-03-classifiability-qa`, base `64ab789`).
...
- **`Sb_CUSTOM_PROGRAM_SCORING_02 — Program Quality Feedback Layer` : ✅ MERGED +
  CANONICAL CI GREEN** 2026-07-23 (build `d0b80cb`, **PR #32 MERGED**, merge `7576600`,
  CI PR 3 jobs verts · CI canonique `29959174506` 3/3 — check externe SonarCloud rouge
  sur `new_coverage` 0.0 % uniquement, artefact connu, **zéro issue de code** vérifiée
  par API)
  (branche `sb/custom-program-scoring-02-feedback`,
...
- **`Sb_CUSTOM_PROGRAM_DOGFOOD_01 — Dogfood du cycle Custom Program` : ✅ MERGED +
  ...
  canonique `31305314688` 3/3 GREEN**, Sonar delta 0, coverage main 91.1 % ; **sous protocole agentique**
  `CLAUDE.md §4` : `GO BUILD` → `GO MERGE si green` ; `docs/SPRINT_Sb_CUSTOM_PROGRAM_DOGFOOD_01_REPORT.md`).
  **Audit produit** du parcours complet
  ...
  **Zéro migration · zéro table · zéro exposition `/library` · zéro changement router/service · zéro
  ...
  ciblé **321** ; check_scope **ISOLATED** ; budget **543 ≤ 548** ; spec PASS. Nettoyage branche/worktree
  = **GO humain séparé (non fait)**.
- **`Sb_CUSTOM_PROGRAM_PUBLICATION_04 — Cycle de republication sûr` : ✅ MERGED +
  ...
  protocole agentique** `CLAUDE.md §4` : `GO BUILD` → préflight → **STOP + arbitrage (Option A)** →
  `GO MERGE` ; `docs/SPRINT_Sb_CUSTOM_PROGRAM_PUBLICATION_04_REPORT.md` +
  ...
  **Décision schéma (Option A opérateur)** : le lifecycle requis est **déjà exprimable** — slugs
  ...
  budget **543 ≤ 548** ; spec PASS. **CI PR #57 5 checks verts · canonique `31302654840` 3/3 GREEN sur
  `1648a03`** ; **Sonar delta 0, fichier neuf 0 issue, coverage main 91.1 %** (zéro code prod). Nettoyage
  branche/worktree = **GO humain séparé (non fait)**.
...
- **`Sb_CUSTOM_PROGRAM_PUBLICATION_02 — Nouveau cycle d'édition post-publication` : ✅ MERGED +
  ...
  **14 tests** + broad sweep ciblé **171** ; check_scope **SHARED_CODE**. **CI 1ᵉʳ passage rouge sur
  `python:S1192`** (littéral 404 dupliqué) → **fix in-scope** (constante `_NOT_FOUND`, `9efcf7a`) →
  re-CI 5/5 verts → mergé. `PUBLICATION_03` (UI templates user) = NEXT / NOT OPENED.
...
- **`Sb_CUSTOM_PROGRAM_WIZARD_06 — Controlled Regeneration` : ✅ MERGED +
  ...
  **Zéro migration · zéro service · générateur / drafts / quality non modifiés · zéro review · zéro
  `WorkoutTemplate`.** 111 tests verts (5 fichiers) ; le « flow de regénération » annoncé comme
  `WIZARD_05+` est **livré**. **CI PR #44 5 checks verts · canonique `31082333554` 3/3 · `new_coverage
  100 %` · Sonar `total: 0` · gate `CLEAN` sans `--admin`** (1ʳᵉ PR de code au gate propre post-fix P0).
- **`Sb_CUSTOM_PROGRAM_WIZARD_05 — EKB-Assisted Exercise Picker` : ✅ MERGED +
  ...
  (`@lru_cache`, lit le JSON, zéro DB) → `picker_options()` (103 noms + métadonnées) + `enrich(name)`
...
- **Nouveau track — Morphology Program** : `Sx_MORPHO_PROGRAM_01` (architecture, **SPEC-ONLY**) = ✅
  **MERGED + VERIFIED** (`8d1ce98`). File de build (chacun sur GO explicite, dans l'ordre des dépendances) :
  ...
  2. **`Sb_PROGRAM_SLOT_INTENT_01`** — `SlotIntent` + résolution via `compute_proximity` (substitution inchangée) = ✅ **MERGED + CANONICAL CI GREEN** (**PR #63**, merge `2c989bd`, CI canonique `31329924105` 3/3 ; `app/services/slot_intent.py` pur, réutilisation lecture seule de `compute_proximity`, `substitution.py` non modifié, 12 tests + broad sweep substitution 123 vert) ;
  3. **`Sb_MORPHO_PROGRAM_GENERATOR_01`** — générateur déterministe morpho additif + availability Fitness Park = ✅ **MERGED + CANONICAL CI GREEN** (**PR #64**, merge `7570817`, CI canonique `31334144907` 3/3, `morpho_program_generator.py` 0 issue Sonar, coverage main 91.4 % ; `app/services/morpho_program_generator.py` pur/déterministe : descripteurs/priorités → `SlotIntent`s ordonnés + exercices EKB candidats via `compute_proximity` **lecture seule** + pool `exercise_properties` + availability, `generated_program_id` = **content-address complet** sha256 `mpg1-` ; **zone macro `lower` désambiguïsée par `muscle_group`** → mollets absents du pool = **warning de couverture, 0 fabrication** ; 19 tests + broad sweep ciblé 180, **substitution N1/N2/N3 inchangée** ; 0 DB/migration/publication/hardcoding Martin ; 2 incidents CI PR résolus in-scope : gate S9073 `95113e7` + thread Gitar fingerprint `fdc93a9`) ;
  4. **`Sb_MARTIN_PROGRAM_01`** — fixture dogfood privée Martin + programme dérivé = ✅ **MERGED + CANONICAL CI GREEN** (**PR #65**, merge `364ea6a`, CI canonique `31359840845` 3/3, coverage main 91.4 % ; dérivation **pure/privée** via le générateur : fixture `tests/fixtures/dogfood/martin_program.py` + doc dogfood + 11 tests + broad sweep 122 ; programme `mpg1-391c65154b3ed546`, 8 intentions Full Body Morphotype, **3 remplis / 5 warnings de couverture, 0 fabrication** ; 0 persistance/DB/Custom/publication/EKB/donnée globale ; **FINDING** : les priorités morphotype de Martin — latéraux/postérieurs/mollets — ne sont **pas couvertes** par `exercise_properties`, d'où le besoin d'un build de **couverture `exercise_properties`** avant l'item 5) ;
  4bis. **`Sb_MORPHO_POOL_COVERAGE_01`** — couverture `exercise_properties` des priorités morphotype (résout le finding item 4) = ✅ **MERGED + CANONICAL CI GREEN** (**PR #67**, merge `16a00d1`, CI canonique `31388403040` 3/3, gate Sonar PR OK `new_coverage 100 %`, coverage main 91.5 %, full sweep 2935 ; fix Gitar `e9bd4d7` : affectation distincte par **couplage biparti maximum** réclamé par `priority_level` — le greedy pouvait affamer un slot ; +16 entrées `exercise_properties` sur noms EKB existants — E4–E8 prescrits+substituts : posterior/latéraux/rear/mollets — + désambiguïsation générateur `shoulders` par `muscle_group` + règle no-duplicate ; `exercise_knowledge_base.json` 16 records `gap→covered`, compteurs QA re-basés (covered 67 / gap 36 / props 69 / blackholes 12) ; **programme de Martin désormais 8/8 remplis, 8 distincts, 0 warning** `mpg1-eadcab6e2d104c45` ; générateur+martin 33 · EKB 32 · substitution 58 ; **substitution N1/N2/N3 inchangée**, 0 expansion EKB, 0 mutation `reference_split`, quad intact) ;
  5. **`Sb_MORPHO_DOGFOOD_01`** — dogfood réel via le cycle Custom existant = ✅ **MERGED + CANONICAL CI GREEN** (**PR #68**, merge `84d54e6`, CI canonique `31397026757` 3/3, gate Sonar PR OK `new_coverage 96.9 %`, `morpho_program_draft_mapper.py` 0 issue Sonar, coverage main 91.5 %, full sweep 2959 ; **dernier build de la file**). Mapper **pur additif** `morpho_program_draft_mapper` : `GeneratedProgram` → charge utile `replace_draft_tree`, volumes hérités du programme catalogue (E1-E8, pinnés), traçabilité `generated:morpho:{intent_id}`. Cycle **réel** traversé par les services existants : draft → validate → **préview qualité 0 écriture** → publish (1 séance → 1 template `user`, freeze + 1 trace qualité) → **lancement propriétaire 303**, non-propriétaire refusé, `/library` liste+slug **404**, **0 template catalogue global**, versioning v2 + republication **inchangés**. **0 nouvel état · 0 table/migration · 0 modif substitution/générateur/session_builder · 0 donnée Martin hors fixture privée.** 24 tests + full sweep. **Limites documentées** : gastroc/soléaire inversés (taxonomie `calves` unique → build dédié sur GO) ; repos non représentable (aucun champ `rest`) → omis, jamais inventé.
- ✅ **Track morpho CLOS** : la file `Sx_MORPHO_PROGRAM_01` est **complète** (spec + profil + slot intent + générateur + programme Martin + couverture du pool + dogfood du cycle réel — tous mergés, CI canonique verte). Candidat de suite identifié par le dogfood, **non ouvert** : **affiner la taxonomie mollets** (`calves_gastrocnemius` / `calves_soleus`) pour que le couplage distingue les deux mécaniques ; la représentation du temps de repos reste hors modèle.
- 🧭 **ROADMAP CANONIQUE (nouvelle référence)** — `Sx_AUREN_ORCHESTRATOR_01_GAP_CONSOLIDATION_SPEC` (✅ **MERGED** 2026-08-10, **PR #69**, merge `d69a1a6`, docs-only ; CI de PR 5/5 verte, CI canonique légitimement skippée par `paths-ignore: docs/**`). **C'est désormais le document de référence pour l'ordre de build** : il réconcilie l'existant réellement livré, le blueprint « orchestrateur d'entraînement personnel », la file morphologie, l'Exercise System V2, `Sx_30`, Body Intelligence, recommandation/readiness/cardio et la fondation zones/muscles — **sur un audit du code actif**, pas des anciens rapports. Il **ne réécrit aucune roadmap** : les sections ci-dessous restent l'historique. **À retenir** : (a) **`Sb_32.4` est une fondation bloquante** — `BodyZone`/`ExerciseMuscleMapping` ne sont **pas** la source de vérité (**un unique chemin d'intégration DB étroit** — lecteur `body_map_descriptor`, plusieurs points d'appel, une seule surface servie — contre 12+ consommateurs lourds en sous-chaîne/JSON) ; (b) **2 défauts vivants découverts et placés en P0** — projection de zones faussée (`profile_metrics` : seul `pecs` comptable → coach et radar BI faux) et collision d'échelle `fatigue_score` (0-100 produit / 0-1 lu, tests verrouillant l'erreur) ; (c) readiness/fatigue/récupération **fragmentés** (5 calculs, 6+ échelles, arête unique *fail-open*) → contrat unique à concevoir (`Sx_RECOVERY_READINESS_01_SPEC`) ; (d) **cardio déconnecté** du modèle de récupération ; (e) **moteur morphologie orphelin** (aucun routeur, `wingspan_cm` inexistant) ; (f) **`recommendation.py` reste non modifiable** — toute planification passe par un service neuf ou un wrapper. **File** : P0 (**2 hotfix** · `Sb_32.4` · `Sx_RECOVERY_READINESS_01_SPEC` — le dogfood morpho est **livré**) → P1 (préférences · budget volume **en plage** · planificateur · pont cardio-fatigue · estimation de récupération · replanification · capture/runtime/explicabilité morpho · activation BI) → P2 (convergence physique-BI · raison de substitution · préférences d'exercices · analytique · explainer) → `Sb_ORCHESTRATOR_E2E_01`. **Chaque slice reste sur GO explicite.** **Premier train validé par l'opérateur (aucun ouvert)** : `Sb_ZONE_COUNT_TAXONOMY_FIX_01` → `Sb_FATIGUE_SCALE_FIX_01` → `Sb_32.4` → consolidation readiness/fatigue/récupération → intégration morphologie runtime → orchestrateur déterministe adaptatif → cockpit / couche d'action. **Motif de cet ordre** : migrer les consommateurs vers `BodyZone` **avant** de corriger la projection de zones propagerait une attribution déjà prouvée fausse — c'est exactement ce qui s'est produit lors du backfill de `exercise_muscle_mappings`, où les collisions connues ont été figées telles quelles.
- 🚄 **TRAIN `AUREN_P0_CORRECTNESS` — EN COURS** (`GO TRAIN` opérateur, base canonique `9f8323f`). Trois **slices de livraison indépendantes**, séquentielles, une PR chacune : `Sb_ZONE_COUNT_TAXONOMY_FIX_01` → `Sb_FATIGUE_SCALE_FIX_01` → `Sb_32.4_BODYZONE_CONSUMER_MIGRATION`. Merge permanent autorisé par slice, cleanup inclus.
  1. **`Sb_ZONE_COUNT_TAXONOMY_FIX_01` (P0.1)** — projection zone détaillée → axe radar. Le défaut de §C.0-A est confirmé **dans le code actif** : `_zone_session_counts` mélangeait 6 axes macro et 11 zones détaillées derrière un garde `if z in counts`, `pecs` étant le seul libellé commun ⇒ **5 axes structurellement figés à 0**. Préflight : un **second exemplaire** du même défaut existait dans `body_intelligence_inputs`, sous forme d'une **table zone→axe dupliquée** (« heuristique V1 ») **déjà divergente** (`glutes`, absente des 11 zones). Correctif : inverse **dérivé** de `RADAR_AXES` (impossible de diverger), projection à **une seule frontière**, une séance comptant **au plus une fois par axe**, table dupliquée **supprimée** (2 → 1). `core` reste sans axe et est abandonnée, jamais fabriquée. **0 taxonomie publique touchée** ⇒ la condition de STOP n'a pas été atteinte. ✅ **MERGED + CANONICAL CI GREEN** (**PR #75**, merge `452c589`, CI canonique `31479653681` 3/3, gate Sonar `OK` `new_coverage 100 %`, 0 thread ; 25 tests, full sweep 3059). **Nouvelle base canonique : `452c589`.**
  2. **`Sb_FATIGUE_SCALE_FIX_01` (P0.2)** — frontière d'échelle de fatigue. Producteur `behavioral` sur **0–100** (`good 20 / flat 50 / fatigued 80`), lecteur `recommendation_explainer` comparant à **0,7 / 0,2** comme si c'était 0–1 ⇒ « fatigue élevée » pour **toute** valeur réelle et branche basse **inatteignable**. Préflight : la seule valeur tombant dans la fenêtre basse était **`0.0`**, précisément ce que `recommendation.py` écrit **quand `compute_behavioral_state` lève** — donc l'app ne disait « bon moment pour pousser » **que lorsqu'elle avait échoué à calculer la fatigue**. Correctif à la **frontière consommateur** (`recommendation.py` **non modifiable**, `behavioral.py` non redessiné) : helper pur `normalize_fatigue_score`, **borné et nommé**, avec un plancher productible **dérivé par test** des dicts de `behavioral` (`(20+10)/2 = 15`, jamais codé en dur) qui identifie `0.0` comme sentinelle d'échec. Borne **volontairement asymétrique** : on refuse d'inventer une bonne nouvelle, on ne supprime pas une mauvaise. Seuils **ancrés, non inventés** : `0.7 × 100 = recommendation.FATIGUE_HIGH_THRESHOLD`, `0.2 × 100 = _GLOBAL_STATE_FATIGUE["good"]`. Tests existants qui **verrouillaient l'erreur** corrigés à l'échelle réelle — assertions conservées, aucune supprimée. ✅ **MERGED + CANONICAL CI GREEN** (**PR #76**, merge `09cd474`, CI canonique `31484328369` 3/3, gate Sonar `OK`, 0 thread ; 46 tests + 24 corrigés, full sweep 3108). **Nouvelle base canonique : `09cd474`.**
  3. **`Sb_32.4_BODYZONE_CONSUMER_MIGRATION` (P0.3)** — fondation bloquante. **Matrice A–E produite avant tout code** (audit du code actif, preuves `fichier:ligne`). **Fait d'identité** : `ExerciseMuscleMapping.exercise_code` contient le **nom** de l'exercice — `SessionExercise.exercise_code_snapshot` (slot `E1…E7`) est un **leurre**. **Nécessité du seed PROUVÉE par la mesure** : `init_db()` → `create_all()` crée les tables de référence **vides** et les backfills Alembic, gardés sur « est-ce moi qui viens de créer cette table ? », ne s'exécutent alors plus jamais — boot applicatif réel = **0/0**, alembic seul = **11/87**, `create_all` puis alembic = l'upgrade **échoue**. Sans seed, migrer un consommateur = le migrer vers une **table vide**, inerte dans 100 % des tests. **Livré** : adaptateur unique `body_zone_source` (correction revue → ligne formelle → repli substring, chaque réponse portant son `resolution_path`, **réutilisant** `_classify_exercise_by_lookup` au lieu d'une 2ᵉ requête) · seed applicatif **déterministe et idempotent** (zones dérivées des dicts `muscle_mapping`, exercices du classifieur canonique ; **rien d'inventé** pour gonfler la couverture) · **1 consommateur lourd migré** : `muscle_scoring._compute_tonnage_by_zone`, la primitive racine du scoring (`recommendation` bloqué par son `@lru_cache`, `session_recap` sans `Session` en signature). **Parité exhaustive sur 105 exercices : 103 exacts, 2 divergences intentionnelles, 0 inexpliquée, 0 ambiguë, 2 manquants rapportés et non mappés.** **2 erreurs connues corrigées avec preuve interne du repo** (rear delt fly : même `machine_slug`/`machine_family` que la variante correcte ; relevé de jambes suspendu : template `liss-abs` de focus « Core / Abdos », siblings tous `core`) — **pas regelées en silence**. Garde `Sb_32.2` **renforcée** : le `git diff HEAD` (vert dès le commit, donc vacant en CI) devient une garde au niveau source qui pinne quel consommateur lit le contrat. ✅ **MERGED + CANONICAL CI GREEN** (**PR #77**, merge `246ed0a`, CI canonique `31495540887` 3/3, gate Sonar `OK`, 0 thread ; 39 tests, full sweep 3147). **2 corrections réelles en revue** : import mort Sonar, et surtout une **régression N+1 introduite par ce sprint** (requête DB dans la double boucle du scoring) corrigée par mémoïsation par nom distinct. **Nouvelle base canonique : `246ed0a`.**
- 🏁 **TRAIN `AUREN_P0_CORRECTNESS` — COMPLET** (2026-08-11). Trois slices indépendantes, trois PR, trois merges, trois CI canoniques vertes : **#75** `452c589` · **#76** `09cd474` · **#77** `246ed0a`. Canonique finale après closeouts. **Prochain train non ouvert** : `Sx_RECOVERY_READINESS_01_SPEC` (P0.4) — **ne pas démarrer sans GO explicite**.
- 📐 **`Sx_RECOVERY_READINESS_01_SPEC` (P0.4) — SPEC ONLY, livrée.** Contrat sémantique canonique readiness / fatigue / récupération / disponibilité, sur **audit du code actif** (17 signaux vivants inventoriés). **4 constats découverts par l'audit** : (a) **le questionnaire de readiness n'influence AUCUNE décision** — `ReadinessEntry` (1–5, persisté, quotidien) ne sort pas du tableau de bord ; déclarer « épuisé » ou « très frais » donne la même recommandation ; (b) **« fatigue » change de direction** — `fatigue_level = 5` veut dire « très frais », `fatigue_score = 80` veut dire « fatigué » ; (c) **« availability » désigne 4 choses** (zone, équipement, **affichabilité d'un bloc UI**, agenda inexistant) ; (d) **2 fail-open vivants** — zone jamais entraînée ⇒ `availability = 1.0`, et `fatigue_score = 0.0` sur exception toujours **produit**. Plus : cardio = île (4 colonnes, 0 contribution), `RECOVERY_HOURS_TARGET` = 4ᵉ copie du dict des 11 zones dans le fichier non modifiable. **Livré** : 5 concepts (`ReadinessSignal` · `FatigueSignal` à 3 composantes · `ZoneRecoveryEstimate` · `AvailabilitySignal` désambiguïsé · `TrainingState` **sans score global unique, contrainte assumée**) · **table de 13 conversions d'échelle nommées** · politique **fail-closed** · cardio V1 **plafonné à confiance `medium`** · **0 taxonomie nouvelle** (réutilise `Sb_32.4` + la projection P0.1) · frontière `recommendation.py` (**lire oui, modifier = HARD STOP**) · test de wording exigé · **file de 5 tranches** · **7 questions ouvertes opérateur**. **Aucune tranche ouverte.** ✅ **MERGED** (**PR #78**, merge `7369141`, 5/5 checks, gate Sonar `OK`, 0 thread, 0 finding Gitar ; **CI canonique légitimement skippée** — merge 100 % `docs/**`, `paths-ignore`, pas un `[skip ci]`). **Nouvelle base canonique : `7369141`** (puis closeout). **File de build P0.4, aucune ouverte, chacune sur `GO BUILD` explicite** : `Sb_RECOVERY_CONTRACT_01` → `Sb_CARDIO_FATIGUE_ADAPTER_01` → `Sb_TRAINING_STATE_AGGREGATOR_01` → `Sb_ZONE_RECOVERY_ESTIMATE_01` → `Sb_RECOVERY_EXPLAINER_01`, puis seulement les consommateurs de planification.
- 🧱 **`Sb_RECOVERY_CONTRACT_01` (P0.4, tranche 1/5)** — types purs et normalizers. **Les 7 décisions opérateur OQ-1..OQ-7 sont enregistrées dans la spec canonique en §12bis**, ajouté **sans réécrire** l'audit d'origine (§12 intact ; le §12bis fait foi en cas de contradiction). **Contrat public** : 3 `StrEnum` + 8 `dataclass(frozen=True)` — `TrainingState` **sans aucun score**, `FatigueSignal` **sans agrégat** (OQ-3 amende ma propre spec §2.2 : `overall`/`as_availability` retirés, le complément devient la fonction pure `fatigue_to_availability`). **13 conversions livrées** + registre `LEGACY_SCALE_CONVERSIONS` liant chaque ligne de spec à son implémentation. **`normalize_fatigue_score` réutilisé, prouvé 3 fois** (espion, garde AST sur tout `app/`, plantation d'une 2ᵉ formule → échec). **Fail-open corrigé dans le vocabulaire** : zone jamais entraînée ⇒ `None`, plus `1.0`. **4 gardes critiques prouvées mordantes par plantation** — dont **une qui ne mordait pas** au premier essai (tolérance trop laxiste sur le vocabulaire interdit), corrigée puis re-prouvée. ✅ **MERGED + CANONICAL CI GREEN** (**PR #79**, merge `fd8df8c`, CI canonique `31532704217` 3/3, gate Sonar `OK`, 0 thread, 0 Gitar ; 186 tests). **Nouvelle base canonique : `fd8df8c`.** **Leçon outillage consignée** : `api/issues/search` de Sonar renvoie `TOTAL 0` sur une PR (auth manquante) — utiliser `api/measures/component_tree` (+ requête directe sur le chemin de test, non renvoyé par `qualifiers=FIL`) ; **MAJOR = 15, CRITICAL = 10**, donc un seul MAJOR casse le gate. ⚠️ **PARTIELLEMENT PÉRIMÉE depuis `Sb_SONAR_GOVERNANCE_01` (2026-08-12)** — le constat « auth manquante » ne vaut plus : la CLI `sonar` authentifiée rend le finding exact (règle + sévérité + fichier) en **un** appel (`sonar list issues --pull-request <N>`). `component_tree` devient un **repli**, plus la route nominale. Les deux pièges (`qualifiers=FIL` ignore les tests ; MAJOR = 15 / CRITICAL = 10) **restent vrais** et servent au repli. Route à jour : skill `auren-sonar-diagnosis`. **Prochaine tranche, non ouverte : `Sb_CARDIO_FATIGUE_ADAPTER_01`.**
- 🚴 **`Sb_CARDIO_FATIGUE_ADAPTER_01` (P0.4, tranche 2/5)** — adaptateur cardio borné. **La correction de préflight opérateur a changé la conception** : le vocabulaire `cardio_machine_type` n'était pas « inconnu » — l'UI est un `<select>` **fermé**, et c'est l'**endpoint** qui est permissif (`routers/sessions.py:628`, **aucune allow-list**). Savoir laquelle des deux garanties tient a dit sur quoi s'appuyer, et a fait **chercher puis trouver** une vraie valeur hors liste dans le dépôt (`stairmaster`) au lieu d'en imaginer. **Production NON AUDITÉE** (aucun accès DB) — dit, pas contourné. **Livré** : `duration / 30 min` clampée, la référence venant du **catalogue** (« 20-30 min LISS »), donc **constante produit et non seuil biologique** · **calories jamais lues** (pas même un paramètre) · **BPM = preuve, jamais magnitude** (aucun ancrage individuel, et le catalogue prescrit la **même** bande 120-130 bpm à tout le monde) · table modalité→zones **heuristique assumée**, rameur seule modalité mixte, elliptique **sans zone haute** (bras non capturés), `autre`/`unknown` **sans zone fabriquée** · **`HIGH` inatteignable** · **aucune décroissance temporelle** (réservée à `Sb_ZONE_RECOVERY_ESTIMATE_01`). **5 garanties prouvées mordantes par plantation.** ✅ **MERGED + CANONICAL CI GREEN** (**PR #80**, merge `790dee0`, CI canonique `31538825629` 3/3, gate Sonar **`OK` au premier passage** — le pré-scan AST avant push a évité les cycles perdus de la PR #79 ; 0 thread, 0 Gitar ; 89 tests). **Nouvelle base canonique : `790dee0`.** **Prochaine tranche, non ouverte : `Sb_TRAINING_STATE_AGGREGATOR_01`.**
- 🧮 **`Sb_TRAINING_STATE_AGGREGATOR_01` (P0.4, tranche 3/5)** — agrégation `TrainingState` en lecture seule sur les entrées persistées. **Le résultat principal est un fail-open attrapé pendant le build** : un utilisateur **sans aucune séance** recevait `strength_component = 0.5`, parce que `behavioral` fabrique `50.0` à partir d'un historique vide et que je l'avais laissé passer. **Une sentinelle de valeur ne pouvait pas suffire** — mesuré : historique vide ⇒ 50.0 (**non productible** par de vraies déclarations), mais séances sans déclaration ⇒ 45.0 (**productible**, donc ambiguë). Correctif : garde d'**existence d'une déclaration** miroitant la sélection du producteur, sans le recalculer. **2 constats d'audit consignés** : le dépôt n'a **aucun** producteur de fatigue dérivé de la charge (`behavioral` = ressentis déclarés), et ce producteur **n'est pas filtré `kind`**. **Livré** : `now` obligatoire (aucune horloge lue), fenêtre 30 j = **périmètre et non décroissance**, **cardio = sélection de la plus récente et non agrégation**, règle de composition **catégorielle** (compte de 4 preuves, aucune arithmétique sur les valeurs, readiness périmée non comptée, `HIGH` jamais produite). **Preuves exécutées** : lecture seule par snapshot DB **contenu compris**, et N+1 par occurrences **×25 à noms constants ⇒ requêtes inchangées**. **`zone_recovery` vide volontairement** ; les faits bruts sortent **hors** de `TrainingState`. **OQ-5 non exercée.** **5 garanties prouvées mordantes par plantation.** **2ᵉ fail-open trouvé par la revue Gitar** : la composante de force **ignorait la fenêtre de 30 j** (`behavioral` n'a aucun filtre de date), donc une déclaration vieille de **400 jours** remontait un état vide à `PARTIAL` — contradiction directe avec la règle « une readiness périmée ne compte pas ». Reproduit, corrigé en bornant la garde par la fenêtre, 2 tests ajoutés. ✅ **MERGED + CANONICAL CI GREEN** (**PR #81**, merge `6d6d1c3`, CI canonique `31575141601` 3/3, gate Sonar `OK` **au premier passage sur les deux poussées**, 0 thread ; 61 tests, broad sweep 589). **Nouvelle base canonique : `6d6d1c3`.** **Prochaine tranche, non ouverte : `Sb_ZONE_RECOVERY_ESTIMATE_01`.**
- 🩹 **`Sb_ZONE_RECOVERY_ESTIMATE_01` (P0.4, tranche 4/5)** — estimation par zone. **OQ-2 appliquée** : la règle temporelle vit dans une **`RecoveryPolicy` versionnée en code**, **pas de colonne `BodyZone`, aucune migration** ; la constante héritée est **lue** via l'adaptateur, jamais copiée, et une `FlatPolicy(version=2)` prouve que la couture de remplacement fonctionne sans toucher au schéma. **Aucune courbe inventée** : l'estimation est la formule héritée (`hours / target`, clampée) atteinte par le contrat ; `decay`/`half_life`/`exp(` interdits par test. **Le fail-open historique est fermé** : une zone jamais vue passe de `availability = 1.0` à `None`/`UNKNOWN`/`NONE`, et un utilisateur neuf reçoit **11 zones explicitement inconnues** plutôt qu'un tuple vide. **Divergences avec l'hérité énumérées et prouvées** (un test recalcule la formule héritée avec la sentinelle `24×365` et vérifie qu'elle rend `1.0`). **Cardio : baisse la confiance, ne lève jamais la disponibilité.** **OQ-5 : roll-up macro de présentation**, pire zone nommée, projection P0.1 réutilisée, **`core` absent de l'axe** mais présent au détail. Branché dans `TrainingState` pour **zéro requête de plus**. **Piège d'import corrigé** : un import différé inutile mettait **deux enums `Confidence` dans un même graphe** sous la purge du conftest — diagnostiqué par `id()` de classe, pas masqué en passant à `==`. **6 garanties prouvées mordantes par plantation.** ✅ **MERGED + CANONICAL CI GREEN** (**PR #82**, merge `609e876`, CI canonique `31598565335` 3/3, gate Sonar `OK`, 0 thread ; 44 + 62 tests, full sweep **3528**). **3 findings de revue, tous de mon fait** : `S9073` dans le **fichier de test** — faille de mon pré-scan, qui ne couvrait que les modules, désormais corrigée et consignée ; plus deux commentaires périmés après le retrait de l'import différé, dont un nommant la direction de dépendance **à l'envers**. Aucun n'affectait le comportement, mais deux faisaient **mentir le code sur lui-même**. **Nouvelle base canonique : `609e876`.** **Dernière tranche, non ouverte : `Sb_RECOVERY_EXPLAINER_01`.**
- 🗣️ **`Sb_RECOVERY_EXPLAINER_01` (P0.4, tranche 5/5 — FILE COMPLÈTE)** — couche de langage utilisateur déterministe. **Première tranche dont la sortie est lue par un humain**, donc le §8 cesse d'être de l'hygiène de source pour devenir de la copie produit. **Décision produit tranchée par l'opérateur et encodée** : *silence sur la physiologie, explicite sur la donnée manquante*. `Confidence.NONE` n'émet **aucune** lecture de récupération — ni rassurante, ni alarmante ; une surface qui attend l'estimation peut dire que la donnée manque, et ce constat vit dans une **collection séparée** avec `recovery_rank_key` qui **lève** dessus : une absence de donnée ne peut pas être triée comme une bande. **Surface détaillée = 11 zones structurellement présentes** (une zone qui disparaît est indiscernable d'une zone oubliée) ; **surface proactive = omission**, et **un seul** message agrégé quand rien n'est estimable, jamais onze. **Cas limite tranché** : une bande exploitable arrivant en `Confidence.NONE` est traitée comme un état de donnée — **c'est la confiance qui décide, pas la bande**. **Confiance ordinale** (« faible » / « moyenne »), jamais un nombre, jamais `%` ; **`HIGH` non rendable** (aucun producteur P0.4 ne l'atteint) et **dégradée** vers le libellé inférieur — sous-estimer est le sens prudent. **Readiness déclarée, jamais mesurée** ; bonne readiness **sans langage d'escalade** (OQ-7). **Cardio = exposition récente** : ni BPM, ni calories, ni durée dans une chaîne rendue ; modalité inconnue ⇒ **aucune zone nommée**. **Traduction des `basis` : table fermée, sinon omission** — un `basis` non reconnu n'est **pas** rendu, et un motif anti-`snake_case` empêche toute fuite de prose interne. **Garde-fou exécuté sur la sortie RÉELLE**, y compris depuis un compte persisté : planter un pourcentage, une revendication de récupération mesurée, un impératif et une fuite d'identifiant fait échouer **10 tests**, dont **2 par le chemin base réelle**. ✅ **MERGED + CANONICAL CI GREEN** (**PR #83**, build `7f363f7` + correctif CI `57325c5` + correctif Gitar `46c7cbd`, merge **`534cbc2`** via `--merge --match-head-commit` **sans squash / sans `--admin` / sans force** ; gate Sonar **`OK`**, couverture du neuf **99.5 %**, **0** smell, **0** bug ; 163 tests dédiés, **full sweep 3691**). **Un échec CI, faux et instructif** : `is` entre deux générations de `recovery_contract` sous la purge `conftest` — **le contrat n'était pas en cause** ; corrigé en résolvant les deux côtés dans la même génération, **sans** relâcher `is` en `==` (ç'aurait masqué la cause). **Leçon de méthode consignée** : 158 tests ciblés **et 1411 de broad sweep étaient verts** — cette classe d'échec n'apparaît que sous l'ordonnancement du **full sweep**, donc un fichier de tests mélangeant imports de module et imports locaux d'`app.*` impose le full sweep **à l'invocation exacte de la CI**, quel que soit le tier. **1 finding Gitar, corrigé au-delà de son impact** : `age_days=None` se rendait en « récemment » — affirmer une fraîcheur que la donnée ne porte pas était la **seule** fabrication restante ; un âge inconnu ou négatif ne produit plus **aucun** mot de temps. **Deux incidents CI d'infrastructure, aucun commit en réponse** : d'abord `Checkout` incapable de joindre `github.com:443` (exit 128, le code n'a jamais tourné) ⇒ re-run des jobs échoués ; puis ce re-run **annulé par concurrence** quand la session parallèle a poussé la PR #84 sur la canonique — la CI faisant autorité pour ce code devient donc le run de `6d16357`, **qui contient ce merge**. **Aucun consommateur branché** : le module n'est importé que par son test, Body Intelligence **n'est pas activée**, l'intégration UI appartient à un sprint consommateur explicite. **🏁 FILE P0.4 COMPLÈTE — 5/5.**
- 🧬 **`Sx_MORPHO_CAPTURE_01` (spec, tranche 1/3 de `AUREN_MORPHO_RUNTIME_01`)** — contrat de capture morphologique. **Livrée pendant que les trains produit sont bloqués**, précisément parce qu'elle n'en dépend pas : audit de code **déjà présent**, diff **100 % `docs/`**, donc `NON_RUNTIME` et **pytest en 9 s** — aucun sweep consommé alors que le plafond mémoire CI reste non résolu. **Le blocage morpho n'était pas le câblage** : (a) **`wingspan_cm` n'a aucune colonne persistée** alors que le moteur dérive l'ape index de `wingspan − height` ⇒ branche entière inatteignable ; (b) **deux écrivains concurrents** sur `body_measurements` — upsert par date vs insertion systématique — jamais arbitrés ; (c) `shoulder_width_cm` capturé et lu par personne. **Tranché** : `create_measurement` écrivain canonique (il porte déjà validation bornée, liste blanche et garde-fou non médical) · append-only **prospectif**, historique **non migré** · `wingspan_cm` colonne **additive nullable sans backfill** (mesurable au mètre ruban, donc dans le périmètre ; l'estimer depuis la taille fabriquerait l'ape index recherché) · `ape_index_cm` **dérivé, jamais stocké** · latéralité = **convention d'agrégation nommée et versionnée**, pas une vérité physiologique · asymétrie G/D **exclue** de `MorphologyFacts` (diagnostic postural interdit) · `calf_cm` hérité **lu, jamais écrit**. **Les 5 interdits durs étendus du moteur à la surface de saisie.** **Limite énoncée** : sans `observations` capturées — et la spec **refuse** d'en ouvrir une surface — le runtime produira des descripteurs à **confiance structurellement réduite**, à savoir avant d'y brancher un planificateur. ✅ **MERGED** (**PR #88**, merge `efef5a2`, 6/6 checks, Sonar `OK`, 0 thread, 0 Gitar ; **CI canonique légitimement skippée** — 100 % `docs/`, `paths-ignore`, pas un `[skip ci]`). **Nouvelle base canonique : `efef5a2`** (puis closeout). **Tranches 2 et 3 NON ouvertes** : `Sb_MORPHO_PROFILE_RUNTIME_01` et `Sb_MORPHO_EXPLAINABILITY_UI_01` sont des tranches runtime dont l'intégration exige un **planificateur qui n'existe pas** (`AUREN_CORE_ORCHESTRATION_01` non démarré, seuil HARD du contrat mémoire CI déjà atteint).
- 🏠 **`Sb_RECOVERY_HOME_CONSUMER_01` (premier consommateur vivant de P0.4)** — la chaîne devient **visible** pour la première fois, via une seule tuile Home « État d'entraînement ». **Additif et explicatif : aucune décision d'entraînement ne bouge** (parité `today`/`week` prouvée **octet pour octet**, tuile active vs neutralisée, sur la même entrée persistée). **L'audit de nommage a été décisif** : la Home portait **déjà deux** surfaces d'état — « État du jour » (widget **déclaratif** 1–5 + CTA de saisie) et « disponibilité » (KPI **hérité** 0–100, l'un des 4 sens d'*availability* de la spec §1). D'où un troisième nom **distinct** plus un sous-titre de provenance (« Estimé à partir de tes séances enregistrées ») : le nom seul ne suffisait pas à distinguer **déclaré / hérité / inféré**. **Pas de HARD STOP** — coexistence lisible, KPI hérité **ni renommé ni retiré** (additif d'abord, OQ-1). **Deux erreurs de conception trouvées par les tests, pas par la lecture** : (a) le **roll-up macro est structurellement muet** — `worst_zone_rollup` dégrade un axe en `Confidence.NONE` dès qu'une de ses 2–3 zones est inconnue, donc sur un compte réaliste les axes ne disent rien et j'aurais livré un bloc vide (comportement **correct** de la tranche 4, pas un défaut) ⇒ la tuile lit des **zones nommées**, plafonnées à 2 ; (b) une séance **musculation ET cardio** ne produit **aucune** estimation (le cardio dégrade la confiance), et ma première version supprimait alors **toute** la tuile — jetant un contexte cardio et un état déclaré **honnêtes** avec l'estimation absente : le principe P0.4 appliqué trop grossièrement. Désormais **message unique d'état de donnée + contexte coexistent** — ce qui doit rester tu, c'est l'**interprétation physiologique**, jamais le **fait enregistré**. **Piège Jinja coûteux** : `.items` sur un dict résout **la méthode**, pas la clé ⇒ Home en **500** ; clé renommée `entries`. Second piège du même rendu : Jinja échappe l'apostrophe (`État d&#39;entraînement`), les assertions comparent le texte **décodé**. **Confinement de panne** via le patron `_safe` existant : tuile indisponible, Home et reco intactes, **jamais** un état favorable substitué. **Une seule** agrégation `TrainingState` par requête, **zéro écriture**, aucun N+1. ✅ **MERGED + CANONICAL CI GREEN** (**PR #85**, build `f1fad86`, merge **`89abeea`**, CI canonique **`31693180062` 3/3**, gate Sonar **`OK` au premier passage**, **100 %** de couverture du neuf, 0 smell, 0 bug, **0 thread**, **0 finding Gitar** ; **73** tests dédiés, broad sweep **1193**, **full sweep 3764**). **Précondition Sonar : `OPERATOR SONAR ACTION STILL REQUIRED`** — New Code toujours `previous_version`/2026-04-10 ; contournement obsolète **non tenté** ; le gate **PR** reste autoritatif et a fonctionné. **Piège d'outillage consigné** : lancer pytest sur des chemins **absolus** de worktree **depuis le cwd du repo principal** fait résoudre `app` depuis le repo principal ⇒ **60 faux échecs, zéro défaut produit** ; `PYTHONPATH` ne suffit pas, il faut `cd <worktree>` et des chemins relatifs. **Nouvelle base canonique : `89abeea`** (puis closeout). **Aucun consommateur supplémentaire ouvert · Body Intelligence toujours non activée · aucun déploiement.**
- 🧭 **`Sb_SONAR_GOVERNANCE_01` (OPS, hors train P0.4)** — **le gate Sonar projet était rouge pour une raison mécanique, pas pour de la dette.** La définition New Code n'avait **jamais** été fixée au niveau projet (`sonar.leak.period`, `parentOrigin: INSTANCE` = `previous_version`) et aucune `sonar.projectVersion` n'est maintenue → fenêtre **gelée au 2026-04-10 pendant 124 jours**, contenant **79 854 lignes « neuves » pour un dépôt de 25 874 lignes (3,1× le projet entier)**. Les PR passaient au vert (périmètre = diff) pendant que le gate projet échouait sur 4 conditions / 6. ⛔ **LA CORRECTION N'A PAS ABOUTI, et c'est le résultat principal de ce sprint.** `api/new_code_periods/set` **n'existe pas sur SonarCloud** (404) ; `settings/set sonar.leak.period=30` est **accepté et persiste à la relecture**, mais l'analyse canonique post-merge (`6d16357`, 2026-08-13T06:50:15Z, ~8 h après l'écriture) rapporte toujours `mode: previous_version` / `2026-04-10` / `new_lines: 81 825`. **Le réglage est inerte** — il a été accepté précisément parce qu'il ne pilote plus rien. Le vrai store exige `/api/v2/new-code-definitions`, qui renvoie **403**. **Permission exacte manquante : `admin` sur le projet, portée par le groupe `Owners` de `mfe-dss`** — `Members` ne porte que `securityhotspotadmin` + `issueadmin`, ce qui explique que l'adjudication d'issues ait fonctionné et pas le réglage. **Action opérateur, non automatisable : UI SonarCloud → Project Settings → New Code → Number of days = 30.** Fuite de portée écartée par preuve : le projet orphelin non touché ne renvoie rien (`{}`), donc l'écriture est bien restée projet-scopée. **2 CRITICAL adjudiqués `FALSE POSITIVE` avec preuve re-vérifiée** : `pythonbugs:S6466` flaggait `bullets[:4]`, une **tranche** (le slicing Python est total, il borne et ne lève jamais) ; `Web:S7930` signalait un id dupliqué dont la « première occurrence » ligne 28 est **à l'intérieur d'un commentaire Jinja `{# … #}`** que l'analyseur Web ne retire pas. `FALSE POSITIVE` et **non** `ACCEPTED` : l'analyseur se trompe sur le code, `ACCEPTED` l'enregistrerait comme ayant raison. **Mesure du stock (779 issues, 2 pages dédupliquées — un appel non paginé en perd 279 en silence)** : **516 ouvertes sur 749 sont `external_*` (68,9 %)**, ingérées depuis `ruff-report.json`/`bandit-report.json`, **qu'aucun Quality Profile Sonar ne peut corriger** — leur pilotage est `pyproject.toml` + `.ruff-budget.json`. **Agent de remédiation : 0/13** (13 branches quotidiennes depuis le 2026-07-31, **0 commit, 0 PR**) — les 4 règles les plus volumineuses (`UP017` 136, `I001` 135, `UP045` 122, `F401` 63) sont toutes `external_ruff`, hors de sa portée structurelle ; **son pilotage n'est pas dans le dépôt** (`sonar.issues.issueResolution.enabled`, `parentOrigin: ORGANIZATION`) → **aucun remplacement inventé**, action opérateur documentée. **Constat canal PR** : sur 6 PR échantillonnées, `sonarqubecloud[bot]` poste **0 commentaire inline** et son résumé de gate arrive **+13 min après la dernière poussée** — il ratifie, il ne déclenche pas ; `gitar-bot[bot]` est le seul à porter des findings localisés. **Stock a11y réel isolé** (13 `InputWithoutLabelCheck` vérifiés légitimes) → **tranche A11Y séparée, non ouverte**. **Lot de résorption de 80 issues Sonar-natives proposé et NON exécuté.** Livré : `docs/ops/SONAR_GOVERNANCE_AUDIT_2026-08-12.md` (source de vérité) · skill `auren-sonar-diagnosis` · liaison VS Code orpheline retirée (non versionnée). **Gate, seuils, required checks, imports ruff/bandit : inchangés. 0 fichier `app/` touché.** ✅ **MERGED + CANONICAL CI GREEN** (**PR #84**, merge `6d16357`, CI canonique `31675257579` **3/3**, gate Sonar PR `OK`, 0 thread, Gitar *Approved, no issues found*). **Effet net mesuré sur l'analyse canonique** : adjudication seule → `bugs` 20 → **18**, `reliability_rating` **D → C**, issues ouvertes 749 → **747**, `FALSE_POSITIVE` 1 → 3 ; New Code **inchangé** ; `ncloc` +476 **appartient à la PR #83 d'une autre session**, pas à ce sprint, et n'a produit **aucune issue nouvelle**. **Incident révélé, non causé par du code** : le merge étant classé `NON_RUNTIME` par `Sb_CI_02_1`, pytest et la couverture sont volontairement sautés → pas de `coverage.xml` → `coverage` et `new_coverage` tombent à **0.0** et le gate gagne une 5ᵉ condition rouge. L'hypothèse de sûreté écrite dans `ci.yml:346-351` (« pas de code neuf ⇒ `new_coverage` non évaluée ») est **vraie au périmètre PR et fausse au périmètre branche tant que la fenêtre New Code est gelée** — les deux défauts se corrigent d'un seul geste (fenêtre 30 j). Transitoire : restauré au prochain merge runtime ; **aucun changement fabriqué n'a été poussé pour forcer une analyse**. **Nouvelle base canonique : `6d16357`** (puis closeout). **NO-GO tacite levé pour rien d'autre : la file P0.4 est close par ailleurs, et ce sprint ne rouvre aucun périmètre produit.**
