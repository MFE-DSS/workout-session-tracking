...
- **Nouveau track — Morphology Program** : `Sx_MORPHO_PROGRAM_01` (architecture, **SPEC-ONLY**) = ✅
  **MERGED + VERIFIED** (`8d1ce98`). File de build (chacun sur GO explicite, dans l'ordre des dépendances) :
  ...
  5. **`Sb_MORPHO_DOGFOOD_01`** — dogfood réel via le cycle Custom existant *(pré-requis recommandé : couverture `exercise_properties` des muscles-priorités morphotype, sinon slots-priorités vides — cf. finding item 4)*.
