...
| **Sx_ASSET_02 Functional Iconography Selection** (2ᵉ cycle `Sx_ASSET`) | 🟢 **SPEC RÉDIGÉE / READY FOR GO COMMIT** 2026-07-20 (docs-only ; `docs/strategy/Sx_ASSET_02_FUNCTIONAL_ICONOGRAPHY_SELECTION_SPEC.md` + rapport + `docs/research/AUREN_ICON_VENDOR_DUE_DILIGENCE.md` ; baseline `c1ad76c`). SEMANTICS BEFORE ICONS : vocabulaire fonctionnel minimal (pas un import de lib). Audit runtime = 8 SVG inline + glyphes/emoji non gouvernés (app quasi sans icônes). Due diligence datée : **Tabler v3.45.0** `975920ff…` MIT (format = contrat Auren) = primaire ; **Health Icons** `891ace7a…` **icônes CC0 / code MIT** = secondaire NOT REQUIRED FOR P0. **P0 = 10 Tabler** (17 noms vérifiés existants) ; P1 différé ; confidence/RIR/métriques typographic-only ; **0 gap custom** → `Sb_ASSET_02.2 NOT REQUIRED`. Versioning épinglé, budget 10 ≤ 20. Rend `ICON INTAKE GATE: READY FOR Sb_ASSET_02.1` ; `ASSET INTEGRATION GATE: BLOCKED` inchangé (0 app/SVG/licence/dépendance). **Prochaine action : `GO COMMIT SPEC — Sx_ASSET_02`** (non commencé). |
