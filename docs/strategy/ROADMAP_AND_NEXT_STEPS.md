...
| Sb_UI_04.3 Active Exercise Cockpit Shell | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-04. Portée : casser la perception de liste → carte active dominante (cockpit) + orientation + exercise intent + **Worked Area Panel textuel minimal dès V1** + up-next + jump bar → mini-stepper compressé. Rupture perceptible exigée (simple recolor/accordéon rejeté). Aucune logique métier touchée. |
| Sb_UI_04.4 Set Logging Console + Progression Guidance | ⏸️ **BLOCKED until .3 delivered and reviewed**. Saisie sets instrumentale + previous perf + target range + hint overload (présentation). |
...
| **Prochaine action** | Recast spec **acceptée** ✅. **Ouvrir `Sb_UI_04.3 Active Exercise Cockpit Shell` SPEC-then-BUILD** sur override explicite opérateur (rupture perceptible exigée, Worked Area Panel textuel dès V1). Options différables non urgentes : release tag baseline preauren, mini-gate 57 kg, cleanup warning bcrypt. |
| Ruff budget | **534 ≤ 548** |
