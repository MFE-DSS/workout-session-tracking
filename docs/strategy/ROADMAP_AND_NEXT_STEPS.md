...
| **Prochaine action** | `Sb_UI_06.2` **accepté** ✅. Ensuite `Sb_UI_06.3` (Home : teaser readiness → pointeur + KPI = sous-ensemble) 🟡 READY TO BE PROPOSED, `.4` (écrans secondaires), **ou** reprise Body Intelligence après stabilisation UI. Autres choix (override) : `Sb_32.4` (bascule consommateurs métier). Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
