...
| Sb_DOGFOOD_01.1 last_time substitution-aware | 🟢 **DELIVERED — pending CI + human review** 2026-07-11 (`docs/SPRINT_Sb_DOGFOOD_01_1_LAST_TIME_SUBSTITUTION_AWARE_REPORT.md`). `last_time_by_exercise_code` (`stats.py`) applique la **même politique substitution que l'overload** (prescrit↔prescrit, substitué↔même substitut, sinon **absence** = silence). Contrat de retour inchangé → 5 surfaces héritent. Matrice S1→S5 (9 tests). Aucun router/overload/modèle/migration/template/CSS/`value=`. `check_scope`=SHARED_CODE → full sweep. ruff 543 ≤ 548 ; 449 ciblés verts. En attente **GO commit** puis CI. |
| **Prochaine action** | `Sb_DOGFOOD_01.1` **livré, en attente GO commit**. Après → CI 3/3, human review. Ensuite `Sb_DOGFOOD_01.2` (vérif silence consommateurs delta/hints/chip/peek en S2/S3/S5) **ou** `Sb_DOGFOOD_01.3` (mobile placeholder CSS). Autres pistes : `Sb_UI_06.4`, reprise **Body Intelligence**, `Sb_32.4`. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
