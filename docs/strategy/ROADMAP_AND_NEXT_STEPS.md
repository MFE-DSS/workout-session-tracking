...
| Sx_UI_04 Session Focus Reskin | 🟢 **SPEC delivered 2026-07-02 — pending human review** (`docs/strategy/Sx_UI_04_SESSION_FOCUS_RESKIN_SPEC.md` + `docs/SPRINT_Sx_UI_04_REPORT.md`). Périmètre reskin verrouillé (7 fichiers autorisés au futur build). Invariants métier verrouillés (services, routes, models, migrations intacts). Bottom nav visible mais discrète pendant focus mode (Option V1a, réversible). Build plan 5 sous-sprints séquentiels `Sb_UI_04.1` → `Sb_UI_04.5`. 9 OQ tranchées V1, 1 pending merge (OQ-H hex contraste réel). **Aucun code touché, aucun CSS modifié, aucun template modifié.** |
| **Prochaine action** | **GO human review Sx_UI_04**, puis choix : (Option A recommandée) ouvrir `Sb_UI_11.1_SCREENSHOT_TOOLING_BUILD` **BUILD** pour livrer baseline P0 avant `Sb_UI_04.1`, OU (Option B) dérogation opérateur explicite pour démarrer `Sb_UI_04.1` sans baseline (non recommandé). **Build UI reste BLOQUÉ** jusqu'à baseline P0 (14 screenshots minimum) OU dérogation. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
