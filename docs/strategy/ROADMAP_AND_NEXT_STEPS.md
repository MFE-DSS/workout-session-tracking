...
| **Prochaine action** | **Capture P0 locale opérateur.** Étapes canoniques : `pip install -e '.[baseline]'` + `python -m playwright install chromium` + création `.env.baseline` (git-ignored) avec fixture user local + `python scripts/visual_baseline_capture.py --priority P0 --viewport all --out-dir var/visual-baseline` + upload artefact CI OU release tag `baseline-preauren-YYYYMMDD`. **Alors seulement** ouvrir `Sb_UI_04.1_CSS_FOUNDATION_BUILD` sur override explicite. **Sb_UI_04.1 reste BLOQUÉ** tant que baseline P0 non capturée localement. **Aucune dérogation baseline n'est accordée.** Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
