...
| Sb_UI_02b.1 Tokens + Home Re-skin | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-07. Tokens Auren Terminal implémentés + re-skin Home graphite/mono/ambre (remplace le patch teal stashé). AA vérifié. Data contract existing-data-only, no-JS, invariants intacts. |
| Sb_UI_02b.2 / .3 · Sb_UI_05.2 | ⏸️ **BLOCKED / DEFERRED**. .2 Focus re-skin · .3 hardening + shell/nav · Sb_UI_05.2 différé derrière la migration visuelle. |
| **Prochaine action** | Sx_UI_02b **accepté** ✅. **Ouvrir `Sb_UI_02b.1 Tokens + Home Re-skin` SPEC-then-BUILD** sur override explicite opérateur (graphite/mono/ambre, remplace le teal). `Sb_UI_05.2` et `Sx_UI_06` différés. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra. |
| Ruff budget | **534 ≤ 548** |
