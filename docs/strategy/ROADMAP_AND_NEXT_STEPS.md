...
| Sb_UI_11.2 Baseline Runtime Integration Patch | ✅ **ACCEPTED 2026-07-02 — CI GREEN + human reviewed** (`docs/SPRINT_Sb_UI_11_2_BASELINE_RUNTIME_PATCH_REPORT.md` + `docs/SPRINT_Sb_UI_11_2_HUMAN_REVIEW_REPORT.md`). Commit `a2846a2`, CI run [`28604484292`](https://github.com/MFE-DSS/workout-session-tracking/actions/runs/28604484292) 3/3 jobs verts (lint 45s / pytest+QA 20min / SonarCloud 1min). Gitleaks / bandit / ruff / pip-audit / spec protocol tous PASS. CLI `visual_baseline_runtime.py prepare/verify` intégré à l'app existant. Refus prod + refus DB non-locale. `--runtime-file` sur capture CLI. 145 tests. **Aucun app UI code touché. Aucun screenshot committé. Aucun runtime artefact committé. Aucun secret leaké.** |
| **Prochaine action** | **Capture P0 locale opérateur via nouveau runtime CLI** : `pip install -e '.[baseline]'` + `python -m playwright install chromium` + `python -m uvicorn app.main:app` (dans un terminal) + `python scripts/visual_baseline_runtime.py prepare --base-url http://127.0.0.1:8000 --out-dir var/visual-baseline` + `python scripts/visual_baseline_capture.py --base-url http://127.0.0.1:8000 --priority P0 --viewport all --out-dir var/visual-baseline --runtime-file var/visual-baseline/runtime.json`. Upload artefact CI OU release tag `baseline-preauren-YYYYMMDD`. **Alors seulement** ouvrir `Sb_UI_04.1_CSS_FOUNDATION_BUILD` sur override explicite. **Sb_UI_04.1 reste BLOQUÉ** tant que baseline P0 non capturée localement. **Aucune dérogation baseline n'est accordée.** Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
