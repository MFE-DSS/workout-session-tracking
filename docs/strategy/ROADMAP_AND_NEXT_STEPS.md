...
| **Prochaine action** | **GO `Sx_UI_04_SESSION_FOCUS_RESKIN_SPEC` SPEC ONLY.** Sur override opérateur explicite. Docs-only strict. La spec peut être écrite, mais le **build `Sb_UI_04.k` reste BLOQUÉ** tant que baseline P0 (14 screenshots) n'est pas disponible OU sans dérogation opérateur explicite. `Sb_UI_11.1` (screenshot tooling build) reste optionnel avant ou après Sx_UI_04 SPEC, mais **obligatoire avant tout build visuel** sauf dérogation. Mini-gate `PROD_DOGFOOD_57KG_LIVE_CHECK` reste à jouer parallèle. |
| Ruff budget | **534 ≤ 548** |
