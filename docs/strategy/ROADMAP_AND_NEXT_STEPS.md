...
| **Prochaine action** | `Sb_UI_06.1` **accepté** ✅. Ensuite `Sb_UI_06.2` (Worked Area : retirer chip zone décoratif + condenser les « à qualifier » vides) 🟡 READY TO BE PROPOSED, `.3` (Home : teaser readiness + KPI), `.4` (écrans secondaires). Autres choix (override) : `Sb_32.4` (bascule consommateurs métier), cycle Body Intelligence. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
