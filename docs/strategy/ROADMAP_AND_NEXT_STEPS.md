...
| Sb_32.3 body_map_descriptor | 🟢 **DELIVERED — pending CI + human review** 2026-07-09 (`docs/SPRINT_Sb_32_3_REPORT.md`). Service **pur, non persistant** `build_body_map_descriptor(name, *, exercise_code=None, db=None) -> dict` (JSON-sérialisable, shape fixe 10 clés). **Aucun modèle/migration/schema/consommateur/UI** (checks migration inchangés verts). `resolution_path` **honnête** (`db_lookup`/`substring_fallback`/`unknown`) : ré-appelle les mêmes helpers Sx_32 dans le même ordre que `classify_exercise` sans le modifier → zones **== baseline**. Labels `BodyZone` si DB sinon `ZONE_LABELS` ; unknown → `"À qualifier"` (aucune anatomie/allégation médicale). **Invariance prouvée : name-only ET db_lookup == baseline 91/91.** ruff 541 ≤ 548 ; spec + migration checks verts ; 16 nouveaux tests + broad sweep 283 verts. Étape 0 Brainstorming documentée. En attente **GO commit** puis CI réelle. |
| Sb_32.4 (ou sprint UI Worked Area) | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-09. Migration consommateurs (coach/body_intel/scoring) vers lookup DB + descriptor, **ou** sprint UI Worked Area consommant le descriptor en SSR. Review-gated, prouve la non-régression (classify old=new) avant bascule. Sur override explicite opérateur. |
| **Prochaine action** | `Sb_32.3` **livré, en attente GO commit** (pas encore commité/pushé). Après GO commit → CI réelle 3/3, puis human review. Ensuite arbitrage : `Sb_32.4` (bascule consommateurs) **ou** sprint UI Worked Area (consommer le descriptor). Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
