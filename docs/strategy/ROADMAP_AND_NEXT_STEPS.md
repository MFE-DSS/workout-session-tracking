...
| Sb_UI_10.3 Public Auth / Welcome Auren Pass | 🟢 **DELIVERED LOCALEMENT — pending GO COMMIT + CI** 2026-07-15 (`docs/SPRINT_Sb_UI_10_3_PUBLIC_AUTH_WELCOME_AUREN_PASS_REPORT.md`). Ouvert avant `10.2` (décision opérateur : `Sb_UI_10.2` **BLOCKED BY ASSETS**, aucun manifest/icon avant gate assets). 8 chaînes visibles migrées sur welcome/login/register (standalone) : apple-titles, titles (dont legacy « · Workout » → « · Auren »), h1 welcome, titre SVG a11y. SPIGNOS = commentaires internes seulement (0 rendu, prouvé client anonyme). Formulaires/routes/erreurs/meta PWA/manifest/CSS intacts ; aucun asset ; zéro Orion. 3 assertions asservies réalignées (invariant Sx_UI_08.2 levé). 15 dédiés + 35 affectés + sweep 180 verts ; ruff 543 ≤ 548 ; spec PASS ; check_scope **ISOLATED non promu** (3 pages bornées, justifié). **En attente GO COMMIT ; CI complète au push.** |
