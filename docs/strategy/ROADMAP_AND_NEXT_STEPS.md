...
| Sb_UI_05.1 Home IA + Hero Decision Surface | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-07. Topologie Home + hero CTA unique (Today Decision Model), bascule board → décision, Auren scoped, no-JS. Data contract existing-data-only. Aucune logique métier touchée. |
| Sb_UI_05.2 → .5 | ⏸️ **BLOCKED until .1 delivered and reviewed**. Active/next cards · readiness/recovery · progress/body continuity · empty states/a11y/hardening. |
| **Prochaine action** | Sx_UI_05 spec **acceptée** ✅. **Ouvrir `Sb_UI_05.1 Home IA + Hero Decision Surface` SPEC-then-BUILD** sur override explicite opérateur. `Sx_UI_06 Exercise Intelligence Presentation` future, not opened. Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warnings infra. |
| Ruff budget | **534 ≤ 548** |
