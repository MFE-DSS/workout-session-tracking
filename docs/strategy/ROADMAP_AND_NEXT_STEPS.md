...
| Sx_UI_08.1 PWA Installability Baseline | 🟢 **DELIVERED — pending GO commit + CI + human review** 2026-07-11 (`docs/SPRINT_Sx_UI_08_1_PWA_INSTALLABILITY_BASELINE_REPORT.md`). **Option A** : manifest resserré (`id`/`lang`/`dir` ; couleurs/noms inchangés) + `base.html` `apple-mobile-web-app-title="SPIGNOS"`. **Pas** de service worker/offline/cache/SPA/JS/nouvelle icône/nouvelle couleur ; assets existants seulement ; no-JS fallback préservé. Session/BI/physique/services/models/migrations/deploy/nginx **non touchés** (compatible dogfooding séance demain). 14 tests dédiés + broad sweep 733 verts. check_scope isolated → promu SHARED_CODE (`base.html` partagé). Limite : pages auth publiques (head standalone) hors périmètre. |
| **Prochaine action** | **GO commit Sx_UI_08.1** attendu (build de code, tier shared_code → CI réelle après push), puis human review. **Séquence recommandée** : (1) **Dogfooding terrain Sx_DOGFOOD_01** (pending demain) — valider les surfaces BI + cohérence charge **avant** de les rendre publiques ; (2) si dogfood concluant → **GO activation** : futur build/ops `Sb_BI_01.activation` (poser `BODY_INTELLIGENCE_ENABLED=1` en prod + ajouter `/body/intelligence` au smoke + deploy `ref=<SHA réel>`) — **GO deploy explicite requis**, smokes jamais sur compte prod. Le plan readiness est prêt (`Sb_BI_01_ACTIVATION_READINESS_PLAN.md`). Autres pistes GO : volume/exercice drill (future spec), corpus improvement (non bloquant). Body Intelligence activation / deploy **deferred until explicit GO**. Pistes hors cycle : `Sb_UI_06.4`, `Sb_32.4`, `Sx_UI_07` (Progress), `Sx_UI_08` (PWA), release tag, smokes UI auth prod. Deferred : release tag, deploy, mini-gate 57 kg, smokes UI auth prod, fix trait sparkline `timeline.py`, `Sb_UI_05.2`, closeout dogfoods Sx_30/31, activation `/body` Manual Profile, rebrand `Sx_UI_10`. À investiguer (non urgent) : hang d'un test au full sweep **local** (n'affecte pas la CI). |
| Ruff budget | **534 ≤ 548** |
