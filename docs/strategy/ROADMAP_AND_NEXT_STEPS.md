...
| Sb_32.1 BodyZone + Muscle Foundation | 🟢 **DELIVERED — pending GO commit + CI + human review** 2026-07-08 (`docs/SPRINT_Sb_32_1_REPORT.md`). Modèles `BodyZone`+`Muscle` (PK int + `code` unique) + migration `j1k6e2f3h54` additive-only : 2 tables neuves + **backfill des 11 zones dérivé à l'exécution** des constantes `muscle_mapping` (jamais recopié ; `core` off-radar → `radar_axis` NULL). `muscles` **vide V1** (aucune anatomie inventée). **Baseline non-régression** `classify_exercise` (91 exercices) figée + testée. `classify_exercise`/consommateurs **inchangés** (bascule DB = `.2`). Whitelist OK ; snapshot régénéré (+6 l. additives) ; drift/snapshot/patterns/roundtrip verts ; ruff 542 ≤ 548 ; **11 nouveaux tests + full sweep 1813 passed**. Étape 0 Brainstorming documentée. En attente **GO commit** puis CI réelle. |
| Sb_32.2 → .4 | ⏸️ **BLOCKED** (séquentiels, review-gated). ExerciseMuscleMapping · body_map_descriptor · migration consommateurs. `Sb_32.2` = bascule `classify_exercise` → lookup DB, prouvée non-régressive contre la baseline `.1`. |
| **Prochaine action** | `Sb_32.1` **livré, en attente GO commit** (pas encore commité/pushé). Après GO commit → CI réelle 3/3, puis human review. `Sb_32.2` reste bloqué tant que `.1` n'est pas accepté. Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
