...
| **Sb_ASSET_03.2 BodyMap Prototype Technical Validation (tentative 1)** | ⛔ **BLOCKED — NON-REPRODUCIBLE OUTPUT** 2026-07-23 (`docs/SPRINT_Sb_ASSET_03_2_BODYMAP_PROTOTYPE_TECHNICAL_VALIDATION_REPORT.md`, **non committé**). Intégrité v1 PASS (taille/SHA/60 entrées, 0 traversée/symlink, 60/60 re-hachées) ; scripts sains (AST) ; **master 66/66**, compact ≤ 12 288 ; **3 méthodes géométriques** (Chrome+Inkscape concordent). **Un seul gate échoue** : reproductibilité — producteur Servier absent du package. Rien copié dans `design/`, rien committé. → corrigé par `Sb_ASSET_03.1-fix`. |
