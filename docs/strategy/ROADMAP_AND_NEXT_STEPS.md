...
| Sb_32.3 body_map_descriptor | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-09. Contrat de représentation corporelle (agrégation zone→descripteur) consommable UI Worked Area + coach, s'appuyant sur `ExerciseMuscleMapping` (`.2`). Review-gated, sous garde de non-régression. Sur override explicite opérateur. |
...
| **Prochaine action** | `Sb_32.2` **accepté** ✅, cycle Sx_32 **in progress**. **Ouvrir `Sb_32.3 body_map_descriptor`** (contrat de service, spec-then-build) sur override explicite opérateur, review-gated, sous garde de non-régression. **Non ouvert dans ce commit.** Options différables : release tag, mini-gate 57 kg, cleanup warnings infra, fix trait sparkline `timeline.py`, `Sb_UI_05.2` (deferred), closeout dogfoods Sx_30/31. |
| Ruff budget | **534 ≤ 548** |
