...
| Sb_ASSET_01.1 Governance Scaffold & Provenance Registry | 🟢 **CODE COMPLETE — CI PENDING — HUMAN REVIEW PENDING** 2026-07-19 (baseline `6167485` ; `docs/SPRINT_Sb_ASSET_01_1_GOVERNANCE_SCAFFOLD_REPORT.md`). 1er build de `Sx_ASSET`. Scaffold `design/auren/` (GOVERNANCE BEFORE ASSETS, 0 asset produit) : README + manifest (16 champs, 8 statuts, 0 entrée `approved`, valeurs UNKNOWN distinctes) + provenance (18 champs, repository-authored, NONE tiers) + style rules (non médical, 11 zones) + intake checklist + LICENSES/README (0 licence tierce). Assets runtime (BodyMap prototype, PWA, icônes inline) référencés sans copie/modification, classés provisional/prototype. 21 tests dédiés (stdlib only) : 0 binaire sous design/auren, 0 master, 0 faux `approved`, assets runtime en place. 0 asset/SVG/PNG/YAML/JSON/licence/dépendance/`app/**`/Custom/métier. check_scope ISOLATED ; ruff clean, budget 543 ≤ 548, spec PASS ; broad sweep 263 verts. Sx_UI CLOSED ; ASSET INTEGRATION GATE BLOCKED. **Prochaine étape : `GO VALIDATE — Sb_ASSET_01.1`** (non commencé). |
