...
| Sb_UI_04.4 Set Logging Console + Progression Guidance | 🟢 **DELIVERED — pending CI + human review** 2026-07-05 (`docs/SPRINT_Sb_UI_04_4_REPORT.md`). Zone set logging → console d'exécution : header + progression X/Y séries + **active set dominant** (fond teal, inputs agrandis, badge « actif ») + **completed ledger** (`✓`, muté) + **upcoming** secondaires + **reference** previous perf (fallback « Non disponible ») + **target** range (overload placeholder / set_scheme / « Objectif à qualifier ») + **guidance** wrapper autour du hint overload (présentation seule). Inputs `set_*_weight_kg`/`_reps` noms/action/method **inchangés** ; placeholder overload re-ciblé sur set actif. SSR/CSS pur, aucun JS, `overload_hint.html` non touché, invariants métier/a11y préservés. 317 tests focus/visual + **32 tests console** verts ; ruff 542 ≤ 548 ; after-capture 16/16 (delta +9.9KB session-detail-active mobile). Whitelist : 1 template + CSS scoped + 2 tests. |
| Sb_UI_04.5 Worked Area Visual Slot + Alternatives + Hardening | 🟡 **next candidate, not opened** (post-Sb_UI_04.4 CI + review). Enrichissement visuel muscle/zone (contrat `body_map_descriptor` §23.5, prêt à brancher) + surface alternatives (substitution) + polish + closure Sx_UI_04. |
| **Prochaine action** | **Vérifier CI verte** sur push Sb_UI_04.4. **Puis human review** (comparaison visuelle after-capture `var/visual-after/Sb_UI_04_4/` vs `Sb_UI_04_3/` — critère : « je pilote mon set courant, plus un formulaire »). **Puis proposition `Sb_UI_04.5 Worked Area Visual Slot + Alternatives + Hardening`** sur override explicite (closure Sx_UI_04). Options différables : release tag baseline preauren, mini-gate 57 kg, cleanup warning bcrypt. |
| Ruff budget | **534 ≤ 548** |
