...
| Sb_UI_11.1 | Screenshot Tooling Build — install Playwright Python + première fixture DB déterministe + `capture.py` initial + baseline P0 (14 screenshots). Touche `scripts/`, `tests/`, potentiellement `requirements-lock.txt` et `.github/workflows/`. **Déclenchera CI complète au push** (comportement voulu). | 🟡 **READY TO OPEN — BUILD** (prochaine action après acceptance Sx_UI_04 push) | — | Précondition dure pour `Sb_UI_04.k` (premier code visuel Sx_UI_04). **Aucune dérogation baseline accordée** — passage par Sb_UI_11.1 obligatoire. |
