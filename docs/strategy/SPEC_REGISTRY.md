...
| Sx_UI_05 | Today / Readiness Home — surface d'entrée mobile-first décisionnelle (« quoi faire aujourd'hui »), Clinical Instrument Auren. 26+ sections : Today Decision Model (CTA unique), Readiness V1 (bande qualitative self-report, **jamais score médical**), active session / next workout, recovery cues légers, progress snapshot + lien, body continuity léger (pas de heatmap). Data contract **existing data only** (sessions/readiness/KPIs/reco/atlas), aucun modèle/migration/service nouveau. Build split 5 sous-sprints. 10 OQ tranchées avec recommandation. | 🟢 **SPEC delivered — pending human review** 2026-07-07 (`Sx_UI_05_TODAY_READINESS_HOME_SPEC.md` + `SPRINT_Sx_UI_05_REPORT.md`). Aucun build ouvert. | Sx_UI_04 CLOSED ✅ + tokens Sx_UI_02 ✅ + app shell Sx_UI_03 ✅ | Non (spec docs-only) ; `Sb_UI_05.k` = Oui (surface uniquement) | [`Sx_UI_05_TODAY_READINESS_HOME_SPEC.md`](Sx_UI_05_TODAY_READINESS_HOME_SPEC.md) + [`SPRINT_Sx_UI_05_REPORT.md`](../SPRINT_Sx_UI_05_REPORT.md) |
