...
| Sb_UI_11.1 | Screenshot Tooling Build — matrice + CLI Playwright + 92 tests unitaires + anti-secret hard rule + `.gitignore` + extra dep `[baseline]`. Aucun app code touché. Aucun screenshot capturé côté sprint. Aucun Chromium installé en CI. **Capture P0 locale à exécuter par opérateur** avec `.env.baseline` fixture. | 🟢 **DELIVERED — pending human review + CI complète attendue** 2026-07-02 | [`SPRINT_Sb_UI_11_1_SCREENSHOT_TOOLING_BUILD_REPORT.md`](../SPRINT_Sb_UI_11_1_SCREENSHOT_TOOLING_BUILD_REPORT.md) | Précondition dure pour `Sb_UI_04.k` (premier code visuel Sx_UI_04) : baseline P0 réellement capturée localement OU dérogation opérateur explicite (non accordée). |
