...
| Sb_32.1 BodyZone + Muscle Foundation | Modèles `BodyZone` + `Muscle` (additive-only) + backfill des 11 zones/labels/measurement/radar/volume depuis `muscle_mapping.py` → base ; test d'invariance ; aucun consommateur muté. | 🟡 **READY TO BE PROPOSED, not opened** | — | DoR : baseline non-régression `classify_exercise` capturée avant bascule. |
