...
|---|---|---|---|---|
| Sx_32 (spec) | Cadrage refonte : dette muscle/zone heuristique (`muscle_mapping.py`, 11 zones hardcodées + substring match) → modèle relationnel `BodyZone`/`Muscle`/`ExerciseMuscleMapping` + contrat `body_map_descriptor` (débloque Sx_UI_04 §23). Invariance historique = contrainte #1. Découpage Sb_32.1→.4 review-gated, migrations additive-only. 7 OQ. Backlog des autres axes Tier 1 documenté (readiness agg, identité exercice, substitution first-class). | ✅ **SPEC HUMAN REVIEW ACCEPTED + CYCLE MÉTIER AUTORISÉ** 2026-07-07 (commit spec `47f3fac`, review docs-only). OQ-32-A → OQ-32-G confirmées (clé `exercise_code` · granularité Zone V1 · muscle_mapping primaire · substitution indépendante · fallback substring conservé · stabilizers « à qualifier » · Worked Area = sprint UI séparé). Invariance historique = contrainte #1 ; non-régression obligatoire ; migrations additive-only. **`Sb_32.1` READY TO BE PROPOSED, not opened.** | [`Sx_32_MUSCLE_BODYZONE_MODEL_SPEC.md`](Sx_32_MUSCLE_BODYZONE_MODEL_SPEC.md) + [`SPRINT_Sx_32_REPORT.md`](../SPRINT_Sx_32_REPORT.md) + [`SPRINT_Sx_32_HUMAN_REVIEW_REPORT.md`](../SPRINT_Sx_32_HUMAN_REVIEW_REPORT.md) | Docs-only, aucun code/migration. Audit backend read-only en source. Cycle autorisé ; build `Sb_32.1` sur override séparé. |
| Sb_32.1 BodyZone + Muscle Foundation | Modèles `BodyZone` + `Muscle` (additive-only) + backfill des 11 zones/labels/measurement/radar/volume depuis `muscle_mapping.py` → base ; test d'invariance ; aucun consommateur muté. | 🟢 **DELIVERED — pending GO commit + CI + human review** 2026-07-08 (`SPRINT_Sb_32_1_REPORT.md`). Modèles `BodyZone`+`Muscle` créés (PK int + `code` unique, convention repo) ; migration `j1k6e2f3h54` (down `7i0f5d1e2g43`) additive-only : 2 tables neuves + backfill des **11 zones dérivé à l'exécution** des constantes `muscle_mapping` (jamais recopié → invariance par construction ; `core` off-radar → `radar_axis` NULL). `muscles` **vide V1** (OQ-32-B/F, aucune anatomie inventée). **Baseline non-régression** `classify_exercise` (91 exercices) figée + testée. `classify_exercise` et tous les consommateurs **inchangés** (bascule DB en `.2`). Whitelist respectée (`muscle_mapping.py`/`seed.py`/services/routes/templates/CSS intacts). Schema snapshot régénéré (+6 l. additives) ; drift/snapshot/patterns/roundtrip verts ; ruff 542 ≤ 548 ; spec protocol vert ; **11 nouveaux tests + full sweep 1813 passed**. Étape 0 Brainstorming documentée. | [`SPRINT_Sb_32_1_REPORT.md`](../SPRINT_Sb_32_1_REPORT.md) | DoR baseline capturée ✅. En attente GO commit puis CI réelle. |
| Sb_32.2 → .4 | ExerciseMuscleMapping → body_map_descriptor → migration consommateurs (coach/body_intel/scoring) | ⏸️ BLOCKED | — | Chaque sous-sprint prouve la non-régression (classify old=new) avant le suivant. |
