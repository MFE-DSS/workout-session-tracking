...
| Sx_UI_04 | Session Focus Reskin — spec ONLY autorisée maintenant, **build reste bloqué** | 🟡 **SPEC ONLY READY TO OPEN** — build `Sb_UI_04.k` requires baseline P0 available (14 screenshots min) OU dérogation opérateur explicite + OQ résiduelles Sx_UI_02 tranchées (OQ-H hex, OQ-I sans, OQ-J mono, OQ-K scale, OQ-M naming) + OQ-R Progression sub-nav | Sx_UI_11 accepté ✅ pour SPEC ; baseline P0 **required first** pour BUILD | Non (spec docs-only), Oui à `Sb_UI_04.k` (surface) | — |
