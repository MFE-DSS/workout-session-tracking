...
| Sb_32.2 ExerciseMuscleMapping + classify lookup/fallback | Table `ExerciseMuscleMapping` + bascule `classify_exercise` → lookup DB avec fallback substring, prouvée non-régressive contre la baseline `.1`. | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-08. Doit reproduire la baseline `classify_exercise` (91 exercices) à l'identique avant tout basculement. Sur override explicite opérateur. | — | Premier vrai basculement de consommateur ; garde-fou = baseline non-régression `.1`. |
| Sb_32.3 → .4 | body_map_descriptor → migration consommateurs (coach/body_intel/scoring) | ⏸️ BLOCKED | — | Séquentiels, review-gated. Chaque sous-sprint prouve la non-régression (classify old=new) avant le suivant. |
