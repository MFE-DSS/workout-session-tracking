...
| Sb_32.3 body_map_descriptor | Contrat de représentation corporelle (agrégation zone→descripteur) consommable UI Worked Area + coach. | 🟡 **READY TO BE PROPOSED, not opened** 2026-07-08. Toujours review-gated, sous garde de non-régression. Sur override explicite opérateur. | — | S'appuie sur `ExerciseMuscleMapping` (`.2`). |
| Sb_32.4 | migration consommateurs (coach/body_intel/scoring) vers lookup DB | ⏸️ BLOCKED | — | Séquentiel, review-gated. Prouve la non-régression (classify old=new) avant bascule. |
