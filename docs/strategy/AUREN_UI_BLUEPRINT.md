...
> | ~~**couverture du classifieur d'exercices**~~ | ❌ **ERRATUM — l'entrée était FAUSSE.** Elle affirmait que « Développé militaire » et « Soulevé de terre roumain » étaient des exercices du catalogue non reconnus. **Ces deux noms n'existent pas dans le catalogue** : ils venaient d'une fixture de mesure. Audit `MUSCLE_MAPPING_COVERAGE_01` : **68/68 exercices actifs mappés en base, 68/68 reconnus par le matcher, 0 non mappé, 0 conflit, 100 % de couverture pondérée programme.** L'affirmation fausse figure dans `c2a5c43` et `a29a104`, conservés comme trace historique. | — |
> | **autorité de mapping inerte** | ⚠ **mesuré** — les 103 lignes actives d'`ExerciseMuscleMapping` n'étaient **jamais consultées** : les sept sites d'appel invoquaient `classify_exercise(name)`. La base est désormais l'**autorité cible** pour l'analytique anatomique via `exercise_zone_resolver` ; la bascule des autres consommateurs reste **différée**. | `EXERCISE_IDENTITY_NORMALIZATION` |
> | **substitution en texte libre** | `sessions.py` accepte un `substituted_name` **libre**, nettoyé à 255 caractères. C'est la **source vivante** de preuve partielle : un nom que le matcher ignore rend l'attribution incomplète. Comportement **inchangé** dans cette tranche. | augmentation future : résoudre vers une identité canonique ou un exercice personnalisé structuré |
